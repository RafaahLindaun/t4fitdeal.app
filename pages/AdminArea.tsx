...
type AdminPrimarySection = "students" | "alerts" | "approvals" | "library" | "templates";
...
export default function AdminArea() {
  ...
  const primaryNavItems: Array<{
    ...
    { id: "students", label: "Alunos", count: students.length, icon: <AdminPeopleIcon size={22} /> },
    { id: "alerts", label: "Alertas", count: dashboardCounts.attention, icon: <AdminWarningIcon size={22} /> },
    { id: "approvals", label: "Aprovações", count: dashboardCounts.pending, icon: <AdminShieldIcon size={22} /> },
    { id: "library", label: "Biblioteca", icon: <AdminDumbbellIcon size={22} /> },
    { id: "templates", label: "Modelos", icon: <AdminSparkIcon size={22} /> },
  ];
  ...
  return (
    <div className="admin-area-screen">
      ...
      <main className={clsx("admin-area-shell", !selectedStudent && "is-dashboard")}>
        {!selectedStudent ? (
          <aside className="admin-dashboard-sidebar" aria-label="Navegação da administração">
            <div className="admin-dashboard-sidebar-brand">
              <AccquaLogo compact />
              ...
              {primaryNavItems.map((item) => (
                <button
                  ...
                  className={clsx(
                    ...
                    activePrimaryNav === item.id && "is-active",
                    (item.id === "alerts" || item.id === "approvals") &&
                      Boolean(item.count) &&
                      "has-alert",
                  )}
                  aria-current={activePrimaryNav === item.id ? "page" : undefined}
            ...
            <div className="admin-dashboard-sidebar-foot">
              <small>{staffLabel}</small>
              <strong>{profile?.fullName || user.email || "Equipe Accqua"}</strong>
              <span>Gestão de alunos e treinos</span>
            </div>
          </aside>
        ) : null}
        ...
        <div className="admin-area-content">
          <header className={clsx("admin-area-header", !selectedStudent && "is-dashboard-header")}>
            ...
            <div className="admin-area-brand admin-v10-brand">
              {selectedStudent ? <AccquaLogo compact /> : null}
        ...
        {!selectedStudent ? (
          <>
            <nav className="admin-dashboard-mobile-nav" aria-label="Seções da administração">
              {primaryNavItems.map((item) => (
                <button
                  type="button"
                  key={`mobile-${item.id}`}
                  className={clsx(
                    "admin-dashboard-mobile-nav-item",
                    activePrimaryNav === item.id && "is-active",
                    (item.id === "alerts" || item.id === "approvals") &&
                      Boolean(item.count) &&
                      "has-alert",
            ...
            <button type="button" className="admin-store-entry" onClick={() => navigate("/area-accqua/loja")}>
              <span><AdminSparkIcon size={22} /></span>
              <div><strong>Loja & catálogo</strong><small>Produtos, reservas e validação das fotos de receitas</small></div>
              <AdminChevronIcon size={18} />
            </button>
                  ...
                  filteredLibraryItems.length ? (
                    <div className="admin-dashboard-resource-grid">
                      {filteredLibraryItems.map((item) => (
                        <article className="admin-dashboard-resource-card" key={item.id}>
                          <span><AdminDumbbellIcon size={22} /></span>
                          <div>
                    ...
                    <div className="admin-area-state">
                      ...
                      <p>Altere a busca para ver outros itens da biblioteca.</p>
                    </div>
                  )
                ) : null}
