...
export default function TestMenu() {
  ...
  const showAdminBanner = profile?.role === "professor" || profile?.role === "admin";
  const showReceptionBanner = profile?.role === "reception";
  ...
  const treinoStatus = useTreinoStatusHoje(user?.id ?? "");
  ...
  const openArea = async (title: string) => {
    ...
    if (title === "Aulas") {
      setMessage("A agenda de aulas será liberada em breve.");
      return;
    }
  ...
  const progressPercentage = workout?.exerciseCount
    ...
    : 0;
  const workoutCompletedToday = treinoStatus.data?.completedToday ?? false;
    ...
    <div className="accqua-menu-screen">
      ...
      <main className="accqua-menu-shell">
        ...
        >
          ...
          <motion.section layout className={`accqua-workout-hero ${workoutCompletedToday ? "is-completed" : ""}`} variants={itemVariants} aria-label="Treino de hoje">
            <div className="accqua-workout-hero-pattern" aria-hidden="true" />
            <div className="accqua-workout-hero-kicker">
              <span>TREINO DE HOJE</span>
              {workoutCompletedToday ? <i>CONCLUÍDO</i> : null}
            </div>

            {homeQuery.isLoading || treinoStatus.isLoading ? (
              <div className="accqua-workout-hero-loading"><span /><span /><span /></div>
            ) : workoutCompletedToday ? (
              <motion.div layout className="accqua-workout-hero-completed">
                <motion.svg viewBox="0 0 48 48" aria-hidden="true" initial={reduceMotion ? false : { scale: .82, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                  <circle cx="24" cy="24" r="20" fill="rgba(255,209,40,.13)" stroke="rgba(255,209,40,.48)" />
                  <motion.path d="M15 24.5l6 6L34 17" fill="none" stroke="currentColor" strokeWidth="3.4" strokeLinecap="round" strokeLinejoin="round" initial={reduceMotion ? false : { pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: .55, ease: [0.2,.8,.2,1] }} />
                </motion.svg>
                <div><h2>Treino de hoje concluído</h2><p>Bom trabalho. Seu progresso já foi atualizado.</p></div>
                <motion.button type="button" whileTap={{ scale: .97 }} onClick={() => navigate("/perfil")}>Ver resumo <MenuArrowIcon size={18} /></motion.button>
              </motion.div>
              ...
              <>
                ...
                <motion.button layoutId="accqua-primary-workout-action" type="button" className="accqua-workout-hero-cta" whileTap={{ scale: 0.97 }} onClick={() => void openWorkout()}>
          ...
          <motion.section className="accqua-streak-card" variants={itemVariants} aria-label="Atividade nos últimos sete dias">
          ...
          <motion.section className="accqua-menu-grid" variants={itemVariants} aria-label="Ações principais">
            {areas.map((area) => {
          ...
          {showAdminBanner ? (
            <motion.button
              type="button"
              className="accqua-membership-card accqua-admin-entry accqua-admin-entry-secondary"
              variants={itemVariants}
              whileTap={{ scale: 0.97 }}
              onClick={() => navigate("/area-accqua")}
            >
              <div className="accqua-membership-icon"><MenuShieldIcon size={27} /></div>
              <div><strong>Área ACCQUA Sports</strong><span>Alunos e montagem de treinos</span></div>
              <MenuArrowIcon className="accqua-admin-entry-arrow" size={20} />
            </motion.button>
          ) : showReceptionBanner ? (
            <motion.button
              type="button"
            ...
            >
              <div className="accqua-membership-icon"><MenuShieldIcon size={27} /></div>
              <div><strong>Atendimento ACCQUA</strong><span>Autorizações e alunos</span></div>
              <MenuArrowIcon className="accqua-admin-entry-arrow" size={20} />
            </motion.button>
          ) : (
            <motion.section className="accqua-membership-card accqua-membership-secondary" variants={itemVariants}>
              <div className="accqua-membership-icon"><MenuShieldIcon size={26} /></div>
              <div><strong>{active ? "Matrícula ativa" : "Acesso liberado"}</strong><span><i className="accqua-status-dot" />{active ? "Acesso liberado" : "Conta autorizada no aplicativo"}</span></div>
            </motion.section>
          )}
        </motion.div>
      </main>
