...
// Login por identificador: o cliente não enumera profiles anonimamente.
...
// Reservas: cancelar mantém histórico, apagar só cancelada, badge conta ativas.
...
requireAll("157/reservation-delete-rpc", migration, [/delete_my_cancelled_reservation_v1_5_7/, /status, ''\) = 'cancelado'/, /revoke all[\s\S]*?from public, anon/], "exclusão não está limitada à reserva cancelada autenticada");
...
// Aulas usa fonte real preenchida em produção.
...
// Alertas de treino: publicação resolve no backend e UI sai uma vez com AnimatePresence.
...
// Staff compartilhado: cabeçalho voltar, auto-collapse e hover leve.
...
// Loading: apenas transform/opacity na animação nova.
...
// Receita IA: TACO em paralelo, Gemini fallback estável e imagem fora do caminho crítico.
