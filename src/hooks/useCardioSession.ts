...
export function useCardioSession({
  ...
  useEffect(() => {
    if (state.phase !== "em_andamento") return;
    if (state.elapsedSeconds <= 0 || state.elapsedSeconds % 5 !== 0) return;
  ...
  const finish = useCallback(async () => {
    ...
    try {
      const result = await finishCardioSession(
        state.session,
        buildSnapshotRef.current(state.elapsedSeconds, "completed"),
      );
      dispatch({ type: "FINISH", validForRanking: result.validForRanking });
      return result;
    } finally {
      setBusy(false);
    }
