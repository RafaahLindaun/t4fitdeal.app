...
export async function setWorkoutStudentAccess(
  ...
  if (rpc.error) {
    ...
    if (
      ...
      technicalMessage.includes("membership_status") ||
      technicalMessage.includes("app_role")
    ) {
      throw new Error(
        "A regra de autorização do Supabase precisa ser atualizada. Execute a migration 20260721_013_corrigir_autorizacao_alunos.sql.",
      );
    }
  ...
  const updatedStudent = await getWorkoutStudentById(studentId);

  if (!updatedStudent) {
    throw new Error(
      "O acesso foi enviado, mas não foi possível confirmar o perfil do aluno.",
    );
  }

  const savedStatus = String(updatedStudent.status)
    .trim()
    .toLowerCase();

  if (savedStatus !== status) {
    throw new Error(
      "A autorização não foi gravada no banco. Execute a migration 20260721_013_corrigir_autorizacao_alunos.sql.",
    );
  }
