...
function normalizeStudentAccessStatus(value: unknown): StudentAccessStatus | null {
...
function accessStatusFromRpcData(data: unknown): StudentAccessStatus | null {
  if (typeof data === "string") return normalizeStudentAccessStatus(data);
  if (Array.isArray(data) && data.length > 0) {
    return accessStatusFromRpcData(data[0]);
  }
  if (data && typeof data === "object") {
    const record = data as Record<string, unknown>;
    return normalizeStudentAccessStatus(
      record.status ?? record.access_status ?? record.membership_status,
    );
  }
  return null;
}

async function readPersistedStudentAccess(
  studentId: string,
  ...
  if (!rpc.error && rpc.data) {
    const row = Array.isArray(rpc.data) ? rpc.data[0] : rpc.data;
    const status = accessStatusFromRpcData(row);
  ...
  if (response.error || !response.data) return null;
  return normalizeStudentAccessStatus(response.data.status);
}
...
export async function setWorkoutStudentAccess(
  ...
  // A autorização possui uma RPC dedicada. Ela atualiza o perfil e também os
  // metadados do Auth, evitando que o aluno continue preso na tela de espera.
  if (status === "active") {
    const authorizeRpc = await supabase.rpc("authorize_accqua_student", {
      p_student_id: studentId,
    });

    if (!authorizeRpc.error) {
      const returnedStatus = accessStatusFromRpcData(authorizeRpc.data);
      const persistedStatus = await readPersistedStudentAccess(studentId);
      if (persistedStatus === "active" || returnedStatus === "active") {
        return "active";
      }
      errors.push("A autorização foi chamada, mas o cadastro continuou pendente.");
    } else {
      errors.push(authorizeRpc.error.message);
    }
  }
  ...
  if (!rpc.error) {
    const returnedStatus = accessStatusFromRpcData(rpc.data);
    const persistedStatus = await readPersistedStudentAccess(studentId);
    if (persistedStatus === status || returnedStatus === status) return status;
    errors.push("O Supabase respondeu sem erro, mas não confirmou a alteração.");
  } else {
    errors.push(rpc.error.message);
  }

  // Compatibilidade com bancos que ainda não receberam a RPC nova. O select
  // após o update impede o falso positivo de uma política RLS que alterou zero
  // linhas sem devolver erro.
  const response = await supabase
    .from("profiles")
    .update({ status })
    .eq("id", studentId)
  ...
  if (!response.error && response.data) {
    const persistedStatus = normalizeStudentAccessStatus(response.data.status);
    if (persistedStatus === status) return status;
    errors.push("O perfil foi localizado, mas o status não foi gravado.");
  } else if (response.error) {
    errors.push(response.error.message);
  } else {
    errors.push("Nenhum perfil foi alterado. A permissão da equipe está bloqueada no Supabase.");
  }

  const uniqueErrors = [...new Set(errors.filter(Boolean))];
  throw new Error(
    uniqueErrors.length
      ? `Não foi possível liberar o aluno: ${uniqueErrors.join(" | ")}`
      : "Não foi possível liberar o aluno. Execute o SQL de autorização da Área ACCQUA.",
  );
}
