...
export async function setWorkoutStudentAccess(
  ...
  if (rpc.error) {
    ...
    if (
      technicalMessage.includes("set_accqua_student_access_v2") ||
      technicalMessage.includes("schema cache") ||
      technicalMessage.includes("function")
    ) {
      throw new Error(
        "Execute a migration 20260722_018_rpc_autorizacao_final.sql no Supabase.",
      );
    }
  ...
  if (response?.persisted !== true) {
    throw new Error(
      "O acesso não foi confirmado na tabela persistente.",
    );
  }
