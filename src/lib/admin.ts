...
export async function setWorkoutStudentAccess(
  ...
  if (rpc.error) {
    ...
    if (
      technicalMessage.includes("set_accqua_student_access_v3") ||
      technicalMessage.includes("schema cache") ||
      technicalMessage.includes("function")
    ) {
      throw new Error(
        "Execute o SQL accqua_autorizacao_persistente_v3.sql no Supabase.",
      );
    }
  ...
  if (normalizedStatus !== status) {
    throw new Error(
      "O Supabase respondeu com um estado diferente do solicitado.",
    );
  }
  ...
  if (
    ...
    response?.profile_persisted !== true ||
    response?.metadata_persisted !== true
  ) {
    throw new Error(
      "O Supabase não confirmou a gravação completa do acesso.",
    );
  }

  const confirmedStudent = await getWorkoutStudentById(studentId);
  const confirmedStatus = String(confirmedStudent?.status ?? "")
    .trim()
    .toLowerCase();
  const normalizedConfirmedStatus =
    confirmedStatus === "ativo" || confirmedStatus === "approved" ||
    confirmedStatus === "aprovado"
      ? "active"
      : confirmedStatus === "pendente"
        ? "pending"
        : confirmedStatus === "inativo"
          ? "inactive"
          : confirmedStatus === "bloqueado"
            ? "blocked"
            : confirmedStatus;

  if (normalizedConfirmedStatus !== status) {
    throw new Error(
      "O acesso foi gravado, mas o perfil do aluno ainda não refletiu a alteração.",
    );
  }
  ...
  return {
    ...
    alreadyAuthorized: Boolean(
      response?.already_authorized ?? false,
    ),
  };
}
