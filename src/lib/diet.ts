...
async function loadRecipeRows() {
  let result = await supabase.from("recipes").select("*").eq("imagem_validada", true).order("nome", { ascending: true }).limit(150);
  if (result.error && /nome/i.test(result.error.message)) {
    result = await supabase.from("recipes").select("*").eq("imagem_validada", true).order("title", { ascending: true }).limit(150);
  }
