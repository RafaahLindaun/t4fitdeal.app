...
function activityValue(value: unknown): CardioActivity {
  ...
  if (normalized === "swim" || normalized === "nado" || normalized === "swimming") {
    return "swim";
  }
  if (
    normalized === "spinning" ||
...
function normalizePrescription(
  ...
  const paceUnit =
    String(raw.pace_unit ?? "km").toLowerCase() === "100m" ? "100m" : "km";
...
export async function loadCardioDashboard(
  ...
  try {
    ...
    const tableError =
      prescriptionResponse.error?.message ??
      sessionResponse.error?.message ??
      "";

    if (
      tableError &&
      /does not exist|relation|schema cache|not found/i.test(tableError)
    ) {
      return {
        prescriptions: [],
        openSession: null,
        ...
        storageReady: false,
        message:
          "Execute a migration de cardio no Supabase para salvar as sessões.",
      };
    }
    ...
    return {
      ...
      storageReady:
        !prescriptionResponse.error && !sessionResponse.error,
      message:
        prescriptionResponse.error?.message ??
        sessionResponse.error?.message ??
        "",
    ...
    return {
      ...
      storageReady: false,
      message:
        "Não foi possível consultar o histórico de cardio agora.",
    };
  }
}
