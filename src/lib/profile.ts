...
async function loadActivityRows(userId: string) {
  // Consulta primeiro a tabela-base. A view antiga também une profiles e,
  // dependendo das políticas RLS existentes no projeto, podia devolver uma
  // lista vazia mesmo quando a sessão já estava registrada.
  const direct = await supabase
...
export async function resolveProfileAvatar(value: string): Promise<string> {
  ...
  const response = await supabase.storage.from("profile-avatars").createSignedUrl(value, 3600);
