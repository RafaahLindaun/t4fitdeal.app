...
export type UpdateStudentMembershipInput = {
  paymentDay: number;
  lastPayment: string;
  validUntil: string;
  notes: string;
  ...
  return new Map(
    ((response.data ?? []) as Record<string, unknown>[]).map((row) => [
      text(row.id),
      {
        ...
        gympassNumber: text(row.numero_gympass),
        membershipValidUntil: text(row.matricula_valida_ate).slice(0, 10),
        membershipPaymentDay: Math.max(0, numberValue(row.matricula_dia_pagamento)),
        membershipLastPayment: text(row.matricula_ultimo_pagamento).slice(0, 10),
        membershipConfirmedAt: text(row.matricula_confirmada_em),
        membershipNotes: text(row.matricula_observacao),
      },
    ]),
  );
}
...
export async function updateStudentMembership(studentId: string, input: UpdateStudentMembershipInput): Promise<StudentMembershipSnapshot> {
  const response = await supabase.rpc("update_accqua_student_membership_v1_4_1", {
    p_student_id: studentId,
    p_payment_day: input.paymentDay > 0 ? input.paymentDay : null,
    p_last_payment: input.lastPayment || null,
    p_valid_until: input.validUntil || null,
    p_notes: input.notes.trim() || null,
    p_confirmed: input.confirmed,
  });
  if (response.error) throw new Error(response.error.message);
  const row = (Array.isArray(response.data) ? response.data[0] : response.data ?? {}) as Record<string, unknown>;
  return {
...
export async function deleteClassScheduleStaff(id: string): Promise<{ action: "deleted" | "deactivated"; reservations: number }> {
  const { data, error } = await supabase.rpc("accqua_staff_delete_class_schedule_v1_3_6", {
    p_schedule_id: id,
  });
  ...
  return {
    ...
    reservations: Math.max(0, Number(raw.reservations ?? 0)),
  };
}
