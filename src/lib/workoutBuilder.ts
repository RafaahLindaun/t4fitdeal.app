...
export type StudentAccessStatus = "pending" | "active" | "inactive" | "blocked";

export async function updateStudentAppAccess(
  studentId: string,
  ...
  const rpc = await supabase.rpc("set_student_app_access", {
    p_student_id: studentId,
    p_status: status,
  });

  if (!rpc.error) {
    return String(rpc.data ?? status) as StudentAccessStatus;
  }
  ...
  if (fallback.error) {
    throw new Error(
      "Não foi possível alterar o acesso. Execute a migration da área da equipe no Supabase.",
    );
  }
