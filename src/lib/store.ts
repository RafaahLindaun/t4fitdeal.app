...
export async function loadStoreProducts(includeInactive = false) {
  ...
  return await productsFromRows((data ?? []) as Row[]);
}
...
export async function saveProduct(input: ProductInput, id?: string) {
  const firstImage = input.images[0]?.url ?? null;
  const payload = {
    ...
    estoque_quantidade:Math.max(0,Math.round(input.stock)), imagem_url:firstImage, ativo:input.active, compra_habilitada:input.purchaseEnabled,
  };
  let productId = id ?? "";
  if (id) {
    const { error } = await supabase.from("produtos").update(payload).eq("id",id); if (error) throw error;
  } else {
    const user = await supabase.auth.getUser();
    const { data,error } = await supabase.from("produtos").insert({ ...payload, criado_por:user.data.user?.id ?? null }).select("id").single();
    if (error) throw error; productId = String(data.id);
  }
