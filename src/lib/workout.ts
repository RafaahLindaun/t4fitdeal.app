...
export async function finishWorkoutSession(input: {
  ...
  if (input.simpleWorkout || !isSupabaseConfigured) {
    return {
      saved: false,
  ...
  const normalizedDuration = Math.max(0, input.durationSeconds);
  const validForRanking = normalizedPercentage >= 70;
  const persistedSessionId = input.localSession
    ? null
    : input.sessionId;

  // A v9.3 conclui sessão + histórico na mesma transação. O aplicativo só
  // informa sucesso quando recebe também o ID do registro do histórico.
  const robust = await supabase.rpc("complete_workout_session_v9_3", {
    p_session_id: persistedSessionId,
    p_plan_id: input.planId,
    p_completion_percentage: normalizedPercentage,
    p_duration_seconds: normalizedDuration,
    p_completed_sets: Math.max(0, input.completedSets),
    p_total_sets: Math.max(0, input.totalSets),
  });

  if (!robust.error && robust.data) {
    const row = Array.isArray(robust.data) ? robust.data[0] : robust.data;
    const result = row as Record<string, unknown>;
    const saved = Boolean(result?.saved && result?.history_id);
    return {
      saved,
      validForRanking: saved && Boolean(
        result?.valid_for_ranking ?? result?.validForRanking,
      ),
    };
  }

  // Compatibilidade: instalações que ainda não receberam a v9.3 tentam a
  // função anterior, mas o sucesso só é aceito após confirmar o histórico.
  const previousV92 = await supabase.rpc("complete_workout_session_v9_2", {
    p_session_id: persistedSessionId,
    p_plan_id: input.planId,
    p_completion_percentage: normalizedPercentage,
    p_duration_seconds: normalizedDuration,
  ...
  if (!previousV92.error && previousV92.data) {
    const row = Array.isArray(previousV92.data)
      ? previousV92.data[0]
      : previousV92.data;
    const result = row as Record<string, unknown>;
    const completedSessionId = String(
      result?.session_id ?? persistedSessionId ?? "",
    );
    const saved = await confirmWorkoutHistory(input.userId, completedSessionId);
    return {
      saved,
      validForRanking: saved && Boolean(
        result?.valid_for_ranking ?? result?.validForRanking,
  ...
  const previousRobust = await supabase.rpc("complete_workout_session_v8_7", {
    p_session_id: persistedSessionId,
    p_plan_id: input.planId,
    p_completion_percentage: normalizedPercentage,
    p_duration_seconds: normalizedDuration,
  ...
  if (!previousRobust.error && previousRobust.data) {
    const row = Array.isArray(previousRobust.data)
      ? previousRobust.data[0]
      : previousRobust.data;
    const result = row as Record<string, unknown>;
    const completedSessionId = String(
      result?.session_id ?? persistedSessionId ?? "",
  ...
  if (!input.localSession) {
    ...
    if (!legacy.error) {
      const saved = await confirmWorkoutHistory(input.userId, input.sessionId);
      return {
        saved,
        validForRanking: saved && Boolean(legacy.data),
      };
    }
    ...
    const saved = !updateError
      && await confirmWorkoutHistory(input.userId, input.sessionId);
    return {
      saved,
      validForRanking: saved && validForRanking,
    };
  }

  // Último fallback para instalações que ainda não executaram a RPC v8.7.
  // Diferente das versões anteriores, nunca informa sucesso sem salvar.
  const completedAt = new Date();
  const startedAt = new Date(
    completedAt.getTime() - normalizedDuration * 1000,
  );
  const direct = await supabase
    .from("workout_sessions")
    .insert({
      ...
      duration_seconds: normalizedDuration,
      valid_for_ranking: validForRanking,
      completed_sets: Math.max(0, input.completedSets),
      total_sets: Math.max(0, input.totalSets),
    })
    .select("id")
    .single();

  const directSessionId = String(direct.data?.id ?? "");
  const saved = !direct.error
    && Boolean(directSessionId)
    && await confirmWorkoutHistory(input.userId, directSessionId);
