...
export async function finishWorkoutSession(input: {
  ...
  const persistedSessionId = input.localSession ? null : input.sessionId;
  ...
  try {
    ...
    if (!response.error && response.data) {
      ...
      const saved = Boolean(result.saved && result.history_id && result.session_id);
      return {
        saved,
        validForRanking: saved && Boolean(result.valid_for_ranking),
        error: saved ? undefined : "O banco não confirmou o histórico do treino.",
      };
    }

    const rpcMessage = String(response.error?.message ?? "");

    // Um único fallback, sem encadear várias funções antigas. Ele conclui a
    // sessão e pede ao banco que crie o histórico canônico da mesma sessão.
    const completedAt = new Date();
    const startedAt = new Date(completedAt.getTime() - durationSeconds * 1000);
    let sessionId = persistedSessionId ?? "";

    if (sessionId) {
      const update = await withRequestTimeout(
        supabase
          .from("workout_sessions")
          .update({
            ...
            valid_for_ranking: percentage >= 70,
      ...
      if (update.error || !update.data?.id) sessionId = "";
    }

    if (!sessionId) {
      const insert = await withRequestTimeout(
        supabase
          .from("workout_sessions")
          .insert({
            student_id: input.userId,
      ...
      if (!insert.error && insert.data?.id) {
        sessionId = String(insert.data.id);
      }
    }

    if (sessionId) {
      const register = await withRequestTimeout(
        supabase.rpc("accqua_register_workout_history_v9_4", {
    ...
    return {
      saved: false,
      validForRanking: false,
      error: rpcMessage || "Não foi possível confirmar o registro no banco.",
    };
  } catch (error) {
    return {
      saved: false,
