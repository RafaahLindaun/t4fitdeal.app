...
export async function finishWorkoutSession(input: {
  ...
  // Esta RPC finaliza a sessão existente ou cria uma sessão concluída quando
  // o início falhou. Ela também grava o histórico usado pelo Perfil e Ranking.
  const robust = await supabase.rpc("complete_workout_session_v9_2", {
  ...
  if (!input.localSession) {
    ...
    if (!legacy.error) {
      return {
        saved: true,
        validForRanking: Boolean(legacy.data),
      };
    }
  ...
  return {
    saved: !direct.error && Boolean(direct.data?.id),
    validForRanking:
      !direct.error && Boolean(direct.data?.id) && validForRanking,
  };
}
