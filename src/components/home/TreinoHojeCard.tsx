...
export default function TreinoHojeCard({
  ...
  const beginCompletion = () => {
    ...
    timerRef.current = window.setTimeout(() => {
      const targetElement = treinoTargetRef?.current;
      ...
      if (targetElement && cardElement) {
        ...
        window.requestAnimationFrame(() => setPhase("hidden"));
        return;
      }
      setPhase("hidden");
    }, 600);
      ...
      <AnimatePresence initial={false} mode="popLayout">
        {phase !== "hidden" ? (
          <motion.section
            ...
            exit={reduceMotion ? { opacity: 0, transition: { duration: 0.12 } } : {
              x: [0, exitTarget.x * 0.3, exitTarget.x],
              y: [0, exitTarget.y * 0.5, exitTarget.y],
              scaleX: [1, 1.15, 0.05],
              scaleY: [1, 0.6, 0.05],
              borderRadius: [24, 40, 999],
              opacity: [1, 1, 0],
          ...
          >
            ...
            {phase === "confirming" ? (
              ...
              <div className="accqua-workout-hero-empty is-error" role="status">
                <h2>Não conseguimos carregar sua ficha agora</h2>
