...
export default function WaterWidget({ currentMl, targetMl, busy, onAdd, onReset, historyAction }: Props) {
  ...
  return (
    <section className={`diet-section-card diet-water-card ${overflowPercent > 0 ? "is-overflow" : ""}`}>
      <header className="diet-card-heading">
        ...
        <div className="diet-card-heading-actions"><strong>{(safeCurrent / 1000).toFixed(1)}L <small>/ {((hasTarget ? safeTarget : 0) / 1000).toFixed(1)}L</small></strong>{historyAction}</div>
      </header>
