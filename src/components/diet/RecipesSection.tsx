...
export default function RecipesSection({ recipes, busy, onRegister }: { recipes: Recipe[]; busy: boolean; onRegister: (recipe: Recipe) => void }) {
  ...
  const visible = useMemo(
    () => filter === "Todas" ? recipes : recipes.filter((recipe) => recipe.tags.some((tag) => tag.toLowerCase().includes(filter.toLowerCase()))),
    [filter, recipes],
  );
          ...
          <article className="diet-recipe-card" key={recipe.id}>
            ...
            <div className="diet-recipe-body">
              <div className="diet-recipe-copy">
                ...
                <small>{Math.round(recipe.macros.calorias)} kcal · P {Math.round(recipe.macros.proteina_g)}g</small>
              </div>
              <motion.button
                type="button"
                disabled={busy}
