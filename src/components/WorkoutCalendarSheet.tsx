...
export default function WorkoutCalendarSheet({
  ...
  const weeklyStatus = useTreinoStatus(userId, { periodo: "semana", reference: now });
  ...
  const trainedThisWeek = weeklyStatus.data?.completedDates.length ?? 0;
      ...
      <section className="workout-calendar-sheet">
        ...
        <main className="workout-calendar-scroll">
          ...
          <div className="workout-calendar-summary">
            <article><span>Dias treinados no mês</span><strong>{monthlyStatus.isError ? "—" : (monthlyStatus.data?.completedDates.length ?? 0)}</strong><small>{data?.currentMonthLabel ?? "Carregando calendário"}</small></article>
            <article><span>Treinos feitos na semana</span><strong>{weeklyStatus.isError ? "—" : trainedThisWeek}</strong><small>{planDays.length ? `${remainingThisWeek} ainda previsto(s)` : "Sem frequência semanal definida"}</small></article>
          ...
          <section className="workout-month-card">
            <div className="workout-month-title"><div><strong>{data?.currentMonthLabel ?? "Calendário mensal"}</strong><span>Os dias amarelos representam treinos concluídos.</span></div><small>{loading || monthlyStatus.isLoading ? "Atualizando..." : monthlyStatus.isError ? "Não foi possível atualizar" : `${trainedDates.size} registro(s)`}</small></div>
