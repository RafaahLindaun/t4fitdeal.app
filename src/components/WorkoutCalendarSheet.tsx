...
import { useTreinoStatusMensal } from "../hooks/useTreinoStatusMensal";
...
export default function WorkoutCalendarSheet({
  ...
  const monthlyStatus = useTreinoStatusMensal(userId, now);
    ...
    <div className="workout-calendar-overlay" role="dialog" aria-modal="true" aria-label="Calendário de treinos">
      ...
      <section className="workout-calendar-sheet">
        <PageHeader
          ...
          left={<span className="workout-calendar-header-balance" aria-hidden="true" />}
          center={<div className="workout-calendar-heading"><span>PLANEJAMENTO DE TREINO</span><h2>Seu calendário</h2></div>}
          right={<button type="button" className="workout-calendar-close" onClick={onClose} aria-label="Fechar calendário"><span className="workout-close-icon" aria-hidden="true" /></button>}
        />
        ...
        <main className="workout-calendar-scroll">
          ...
          <div className="workout-calendar-summary">
            <article><span>Dias treinados no mês</span><strong>{monthlyStatus.data?.completedCount ?? 0}</strong><small>{data?.currentMonthLabel ?? "Carregando calendário"}</small></article>
            <article><span>Treinos feitos na semana</span><strong>{trainedThisWeek}</strong><small>{planDays.length ? `${remainingThisWeek} ainda previsto(s)` : "Sem frequência semanal definida"}</small></article>
          ...
          <section className="workout-month-card">
            <div className="workout-month-title"><div><strong>{data?.currentMonthLabel ?? "Calendário mensal"}</strong><span>Os dias amarelos representam treinos concluídos.</span></div><small>{loading || monthlyStatus.isLoading ? "Atualizando..." : `${trainedDates.size} registro(s)`}</small></div>
