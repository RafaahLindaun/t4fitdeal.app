...
function roleFromEmail(email?: string | null): AppRole | null {
  ...
  if (
    ...
    domain.includes("administracao") ||
    domain.includes("administrativo") ||
    domain.includes("gestao")
  ) {
    return "admin";
  }
  return null;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfile = useCallback(async (user: User | null) => {
    ...
    try {
      const currentProfileRpc = await withTimeout(
        supabase.rpc("get_my_accqua_profile_v2"),
      );
      const legacyProfileRpc = currentProfileRpc.error
        ? await withTimeout(supabase.rpc("get_my_accqua_profile"))
        : null;
      const rpcResult = currentProfileRpc.error
        ? legacyProfileRpc
        : currentProfileRpc;
