...
const isProfessorEmail = (email?: string | null) =>
  Boolean(email?.trim().toLowerCase().endsWith("@professor.com"));
...
function normalizeRole(value: unknown, email?: string | null): AppRole {
  ...
  if (isProfessorEmail(email)) return "professor";
  return "student";
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUpFirstAccess = async (input: FirstAccessInput): Promise<AuthResult> => {
    ...
    const professor = isProfessorEmail(input.email);
    const metadata = {
      ...
      requested_role: professor ? "professor" : "student",
      role: professor ? "professor" : "student",
      status: professor ? "active" : "pending",
    };
    ...
    return {
      message: professor
        ? "Conta de professor criada. Confirme o e-mail e entre no aplicativo."
