...
function calcStreak(workoutDates) {
  ...
  const workoutSet = new Set((workoutDates || []).filter(Boolean));
...
function calcReadiness(lastWorkout) {
  ...
  if (!lastWorkout?.completed_at && !lastWorkout?.finished_at && !lastWorkout?.created_at) {
...
function getRowMinutes(row, fallback = 0) {
...
async function loadPlanStatus(userId) {
  ...
  return loadUserPlanStatus(userId);
...
async function loadCompletedWorkoutDates(userId) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("session_date, completed, completed_at, finished_at, started_at, created_at")
...
async function loadLastWorkout(userId) {
  ...
  try {
    ...
    if (error) {
      ...
      console.error("loadLastWorkout error:", error);
    ...
    console.error("loadLastWorkout catch:", err);
...
async function loadTodayCaloriesSummary(userId, dateKey, weightKg = 80) {
  ...
  try {
    ...
    if (workoutRes.error) {
      ...
      console.error("loadTodayCalories workout error:", workoutRes.error);

    }

    if (cardioRes.error) {

      console.error("loadTodayCalories cardio error:", cardioRes.error);

    }

    const workoutRows = Array.isArray(workoutRes.data) ? workoutRes.data : [];

    const cardioRows = Array.isArray(cardioRes.data) ? cardioRes.data : [];

    const todayWorkoutRows = workoutRows.filter((row) => {

      const sameDay = rowDateKey(row) === dateKey;
    ...
    const todayCardioRows = cardioRows.filter((row) => {
...
export default function Dashboard() {
  ...
  const calorieWave = useMemo(() => {
    ...
    return [
      ...
      clamp(0.2 + streakScore * 0.16, 0, 1),

      clamp(0.3 + kcalScore * 0.22, 0, 1),

      clamp(0.26 + frequencyScore * 0.22, 0, 1),

      clamp(0.34 + streakScore * 0.2, 0, 1),
