...
function ExerciseGif({
  ...
  const candidates = useMemo(() => {
    const manifestMatches = mediaUrl ? matchExerciseMediaFiles(manifestFiles, { mediaUrl, slug, name }) : [];
    return [...new Set([
      ...buildMediaCandidates(mediaUrl, name || slug),
      ...manifestMatches,
    ])];
  ...
  if (failed || !source) {
    return (
      <span
        ...
        role="img"
        aria-label={`Sem mídia cadastrada para ${name}`}
      >
        <AdminWarningIcon size={16} />
        <small>Sem mídia</small>
      </span>
    );
  }
