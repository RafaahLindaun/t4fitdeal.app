...
function gifExtensionCandidates(value: string) {
  const clean = value.trim();
  if (!clean) return [];

  const match = clean.match(/^(.*)\.([gG][iI][fF])([?#].*)?$/);
...
function ExerciseGif({
  ...
  const candidates = useMemo(
    () =>
      Array.from(
        new Set(
          [
            ...gifExtensionCandidates(mediaUrl),
            `/gifs/${slug}.gif`,
            `/gifs/${slug}.GIF`,
            "/gifs/exercise-placeholder.gif",
            "/gifs/exercise-placeholder.GIF",
          ].filter(Boolean),
        ),
      ),
  ...
  return (
    <img
      className={className}
      src={candidates[sourceIndex] ?? "/gifs/exercise-placeholder.gif"}
...
function SwipeToRemove({
  ...
  return (
    <div
      className={`admin-builder-swipe ${offset < 0 ? "is-open" : ""}`}
    >
      <button
        type="button"
        className="admin-builder-swipe-delete"
        onClick={() => {
          close();
          onRemove();
        }}
      ...
      <div
        ...
        onPointerMove={(event) => {
          ...
          if (!horizontalGesture.current) {
            ...
            if (Math.abs(deltaX) > 7) horizontalGesture.current = true;
          }

          if (!horizontalGesture.current) return;
          const next = Math.max(-116, Math.min(0, deltaX));
          currentOffset.current = next;
          setOffset(next);
        }}
        onPointerUp={() => {
          if (!pointerStart.current) return;
          const shouldOpen = currentOffset.current <= -54;
          currentOffset.current = shouldOpen ? -108 : 0;
...
export default function AdminWorkoutBuilder() {
  ...
  useEffect(() => {
    ...
    const loadBuilder = async () => {
      try {
        const [studentResult, libraryResult, templateResult] = await Promise.all([
  ...
  const addCustomExercise = async () => {
    ...
    try {
      const localItem: ExerciseLibraryItem = {
        id: `catalog-custom-${Date.now()}`,
        slug: normalize(customExercise.name)
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/^-+|-+$/g, ""),
        name: customExercise.name.trim(),
        muscleGroup: customExercise.muscleGroup.trim() || "Outros",
        equipment: customExercise.equipment.trim(),
        category: customExercise.muscleGroup.trim() || "Outros",
        mediaUrl: customExercise.mediaUrl.trim(),
        beginnerTip: customExercise.beginnerTip.trim(),
        defaultSets: Math.max(1, customExercise.defaultSets),
        defaultRepsMin: Math.max(1, customExercise.defaultRepsMin),
        defaultRepsMax: Math.max(
          Math.max(1, customExercise.defaultRepsMin),
          customExercise.defaultRepsMax,
        ),
        defaultRestSeconds: Math.max(15, customExercise.defaultRestSeconds),
      };

      const item = customExercise.saveToLibrary
        ? await createExerciseLibraryItem(customExercise)
        : localItem;
      ...
      setToast(
        customExercise.saveToLibrary
          ? "Exercício salvo na biblioteca e adicionado ao treino."
          : "Exercício adicionado somente para este aluno.",
      );
    } catch (error) {
      setToast(
        error instanceof Error
            ...
            <div className="admin-builder-split-options">
              {SPLIT_OPTIONS.filter(
                ...
                <button
                  ...
                  onClick={() => changeSplit(option.code)}
                >
        ...
        >
          <header>
            ...
            <div className="admin-builder-library-header-actions">
              ...
              <button type="button" onClick={openCustomExercise}>
                <AdminPlusIcon size={17} />
                Novo exercício
              </button>
            </div>
          </header>
          ...
          <aside className="admin-builder-custom-sheet">
            ...
            <div className="admin-builder-custom-fields">
              ...
              <label className="is-wide">
                ...
                />
                <small>Aceita arquivos .gif e .GIF que já estejam no public.</small>
              </label>
              ...
              <label className="admin-builder-custom-toggle is-wide">
                <input
                  type="checkbox"
                  checked={customExercise.saveToLibrary}
                  onChange={(event) =>
                    setCustomExercise({
                      ...customExercise,
                      saveToLibrary: event.target.checked,
                    })
                  }
                />
                <span>
                  <strong>Salvar na biblioteca da equipe</strong>
                  <small>O exercício ficará disponível nas próximas montagens.</small>
                </span>
              </label>
            </div>
