  ...
  if (failed || !candidates.length) {
    return (
      <span
        ...
        role="img"
        aria-label={`GIF indisponível para ${name}`}
      >
...
export default function AdminWorkoutBuilder() {
  ...
  const returnToStudent = () => {
    navigate(
      returnStudentId
        ? `/area-accqua/alunos?student=${returnStudentId}`
        : "/area-accqua",
    );
  };
    ...
    <div className="admin-builder-screen">
      ...
      <main className="admin-builder-shell">
        ...
        <div className="admin-builder-mobile-tools">
          <div className="admin-builder-mobile-routines" aria-label="Rotinas do treino">
            {routines.map((routine, index) => (
              <button
                type="button"
                key={`mobile-${routine.code}-${index}`}
                className={activeRoutineIndex === index ? "is-active" : ""}
                onClick={() => {
