...
function statusLabel(status: string) {
  ...
  if (["reserved", "booked", "agendada", "reservada"].includes(normalized)) return "Agendada";
          ...
          <div className="activity-copy">
            ...
            <div className="activity-meta">
              ...
              {activity.kind === "workout" && activity.completionPercentage > 0 ? (
                <i>{Math.round(activity.completionPercentage)}% concluído</i>
              ) : null}
...
export default function Profile() {
  ...
  const upcomingClasses = useMemo(() => {
    ...
    return dashboard.classes.filter((item) => {
      ...
      return scheduled >= now && !["cancelled", "canceled", "cancelada"].includes(item.status.toLowerCase());
    });
  }, [dashboard.classes]);
  ...
  const updatePreferences = async (
    ...
    if (key === "workoutNotifications" && value && "Notification" in window) {
      try {
  ...
  return (
    <div className="accqua-profile-screen">
      ...
      <main className="accqua-profile-shell">
        <header className="accqua-profile-header">
          <button type="button" onClick={goBack} aria-label="Voltar">
            <ProfileIcon name="arrow" size={25} />
          </button>
          <div className="accqua-profile-brand"><AccquaLogo compact /></div>
          <span className="profile-header-space" />
        </header>
        ...
        <div className="accqua-profile-title-row">
          ...
          {view === "main" ? (
            <button type="button" className="profile-edit-shortcut" onClick={() => openView("personal")}>
              <ProfileIcon name="edit" size={19} />
            </button>
          ) : null}
        </div>

        <section className="accqua-profile-content">
          {loadingDashboard ? (
            <div className="accqua-profile-loading">
              <span />
            ...
            <>
              <section className="profile-identity-card">
                <div className="profile-avatar">
                  {details.avatarUrl ? <img src={details.avatarUrl} alt="Foto do perfil" /> : <strong>{initials(details.fullName)}</strong>}
                  <span className={active ? "is-active" : ""} />
                </div>
                <div className="profile-identity-copy">
                  <h2>{details.fullName || "Aluno ACCQUA"}</h2>
                  <p>{formatMemberSince(details.memberSince)}</p>
                  <div className="profile-identity-tags">
                    <span className="is-status"><i />{active ? "Matrícula ativa" : "Conta autorizada"}</span>
                    {details.registrationCode ? <span>#{details.registrationCode}</span> : null}
                  </div>
                </div>
              </section>

              <section className="profile-stats-grid" aria-label="Resumo do perfil">
                <article><span><ProfileIcon name="dumbbell" /></span><strong>{dashboard.totalWorkoutSessions}</strong><p>Treinos</p></article>
                <article><span><ProfileIcon name="cardio" /></span><strong>{dashboard.totalCardioMinutes}</strong><p>Min. cardio</p></article>
                <article><span><ProfileIcon name="calendar" /></span><strong>{dashboard.totalClasses}</strong><p>Aulas</p></article>
                <article><span><ProfileIcon name="trophy" /></span><strong>{dashboard.rankingPoints}</strong><p>Pontos</p></article>
              </section>
          ...
          {!loadingDashboard && view === "personal" ? (
            <form className="profile-form" onSubmit={savePersonal}>
              <section className="profile-detail-hero">
                <span className="profile-avatar is-small">{initials(personalForm.fullName)}</span>
          ...
          {!loadingDashboard && view === "settings" ? (
            <section className="profile-subview">
              <div className="profile-toggle-card">
                <ProfileToggle title="Aparecer no ranking" subtitle="Mostra apenas seu primeiro nome" checked={personalForm.showInRanking} onChange={async (value) => { const next = { ...personalForm, showInRanking: value }; setPersonalForm(next); const result = await savePersonalProfile(next); setMessage(result.error ? "Não foi possível salvar." : "Configuração salva."); }} />
              ...
              <div className="profile-settings-note"><ProfileIcon name="shield" size={23} /><div><strong>Privacidade protegida</strong><p>CPF, telefone e e-mail não aparecem no ranking nem para outros alunos.</p></div></div>
            </section>
          ) : null}
        ...
        <BottomNavigation activeKey="perfil" onSelect={(label) => void handleBottomNavigation(label)} />
      </main>
