...
function RankingInfoSheet({
  ...
  const prizeQuery = useQuery({
    queryKey: ["ranking-config", "nome-premio"],
    queryFn: loadRankingPrizeName,
    enabled: open,
    staleTime: 5 * 60_000,
  });
  ...
  return (
    <ResponsiveDialog
      ...
      ariaDescriptionId="ranking-info-description"
      className="ranking-info-responsive-dialog"
      bodyClassName="ranking-info-dialog-body"
      closeButton={<button type="button" className="ranking-sheet-close" aria-label="Fechar informações do ranking">×</button>}
    >
      <div className="ranking-info-list">
        <article>
          <strong>Treinos do mês</strong>
          <p>Sua posição é definida pela quantidade de treinos registrados no mês atual.</p>
        </article>
        <article>
          <strong>Prêmio para o 1º lugar</strong>
          <p>Quem terminar o mês em primeiro lugar ganha: {prizeName}.</p>
        </article>
        <article>
          <strong>Todo mês começa do zero</strong>
          <p>A contagem é mensal. No início de cada mês o ranking começa um novo ciclo para todos os alunos.</p>
        </article>
      </div>
    </ResponsiveDialog>
  );
