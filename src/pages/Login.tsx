...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const syncViewport = () => {
      ...
      const stableHeight = Math.max(window.innerHeight, document.documentElement.clientHeight);
      const keyboard = stableHeight - height > 120;

      document.documentElement.style.setProperty("--app-viewport-width", `${width}px`);
      document.documentElement.style.setProperty("--app-viewport-height", `${height}px`);
      document.documentElement.style.setProperty("--login-artboard-scale", String(Math.max(0.52, fitScale)));
      document.documentElement.style.setProperty("--login-width-scale", String(Math.max(0.68, widthScale)));
  ...
  return (
    <div className={`login-screen concept-login ${keyboardMode ? "keyboard-open" : ""}`}>
      ...
      <main className="login-artboard" aria-label="Login Accqua Sports">
        ...
        <form className="concept-login-form" onSubmit={handleLogin}>
          ...
          <button className="concept-forgot-below" type="button" onClick={handleReset}>
            Esqueci minha senha
          </button>
      ...
      {registerOpen ? (
        <div className="onboarding-overlay" role="dialog" aria-modal="true" aria-label="Primeiro acesso">
          <section className={`onboarding-card ${keyboardMode ? "keyboard-visible" : ""}`}>
              ...
              {registerStep === 3 ? (
                <div className="onboarding-step review-step">
                  ...
                  <div className="register-note">
                    ...
                    <br />
                    <strong>Professor:</strong> mantém a lógica de autorização já configurada no Supabase.
                  </div>
                </div>
              ) : null}
            </div>
