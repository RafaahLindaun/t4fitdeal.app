...
export default function ClassesAdmin() {
  ...
  const [scheduleDeleteTarget, setScheduleDeleteTarget] = useState<ClassSchedule | null>(null);
  ...
  const deleteScheduleMutation = useMutation({
    mutationFn: (id: string) => deleteClassScheduleStaff(id),
    onSuccess: async (result) => {
      toast.success(result.action === "deleted" ? "Horário excluído." : "Horário desativado para preservar reservas existentes.");
      await queryClient.invalidateQueries({ queryKey: ["classes-management"] });
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível excluir o horário."),
  });
  ...
  return (
    <div className="classes-admin-screen">
              ...
              {(data?.types ?? []).map((type) => {
                ...
                return (
                  <motion.article layout className={clsx("classes-admin-type-card", !type.active && "is-inactive")} key={type.id}>
                    ...
                    {isOpen ? (
                      <motion.div
                        className="classes-admin-type-body"
                      ...
                      >
                        ...
                        <div className="classes-admin-schedules">
                          {schedules.length ? schedules.map((schedule) => (
                            <SwipeableListItem key={schedule.id} onDelete={() => setScheduleDeleteTarget(schedule)} deleteLabel="Excluir horário">
                              <div className={clsx("classes-admin-schedule", !schedule.active && "is-inactive")}>
                          ...
                          )) : <EmptyState icon={<AdminCalendarIcon size={25} />} title="Nenhum horário cadastrado" description="Use “Adicionar horário” e selecione vários dias de uma vez." />}
                        </div>
                      </motion.div>
                    ) : null}
                    </AnimatePresence>
      ...
      <ConfirmDeleteDialog open={Boolean(scheduleDeleteTarget)} onOpenChange={(open) => { if (!open) setScheduleDeleteTarget(null); }} title="Excluir horário?" description="Se este horário já possuir reservas, ele será apenas desativado para preservar o histórico dos alunos. Sem reservas, será removido definitivamente." onConfirm={async () => { if (!scheduleDeleteTarget) return; await deleteScheduleMutation.mutateAsync(scheduleDeleteTarget.id); setScheduleDeleteTarget(null); }} busy={deleteScheduleMutation.isPending} />
