...
export default function AdminArea() {
  ...
  const canApproveAccess =
    profile?.role === "reception" || profile?.role === "admin";
  const staffLabel =
    profile?.role === "admin"
      ? "ADMINISTRAÇÃO"
      : profile?.role === "reception"
  ...
  useEffect(() => {
    ...
    void Promise.all([
      loadExerciseLibrary(),
      loadAdminProfessors(),
    ]).then(([exerciseLibrary, staffProfessors]) => {
      setLibrary(exerciseLibrary);
      setProfessors(staffProfessors);
    });
  ...
  useEffect(() => {
    ...
    setSelectedProfessorId(
      selectedStudent.linkedProfessorId ||
        (profile?.role === "professor" ? user?.id ?? "" : ""),
    );
  }, [
    profile?.role,
    selectedStudent?.id,
  ...
  const linkSelectedStudent = async (
    ...
    if (!selectedStudent || !professorId || linkSaving) return;
    setLinkSaving(true);
    ...
    try {
      ...
      setSelectedProfessorId(linkedStudent.linkedProfessorId);
      setReloadKey((current) => current + 1);
      setToast(
        `Aluno vinculado a ${
          linkedStudent.linkedProfessorName || "professor"
        }.`,
      );
    } catch (error) {
      setToast(
  ...
  const authorizeStudent = async () => {
    ...
    if (!canApproveAccess) {
      setToast("Somente administração ou recepção pode liberar a entrada no aplicativo.");
      return;
    }
    ...
    try {
      ...
      if (
        profile?.role === "professor" &&
        user?.id &&
        !authorizedStudent.linkedProfessorId
      ) {
        try {
          ...
          refreshStudent(authorizedStudent);
          linkNotice = " O aluno também foi vinculado a você.";
        } catch {
          linkNotice =
            " O acesso foi liberado; confirme o vínculo do professor abaixo.";
        }
      }
  ...
  const blockStudent = async () => {
    if (!selectedStudent || accessSaving) return;
    if (!canApproveAccess) {
      setToast("Somente administração ou recepção pode bloquear o acesso.");
      return;
    }
              ...
              <div>
                ...
                <p>
                  {canApproveAccess
                    ...
                    : "Consulte o perfil completo e organize o treino dos alunos já autorizados pela administração."}
                </p>
              </div>
            </section>
          ...
          <section className="admin-profile-view">
            ...
            <article className="admin-access-card">
              ...
              {canApproveAccess ? (
                ...
                <button
                  ...
                  disabled
                  title="A liberação de entrada é exclusiva da administração e da recepção."
                >
                  <AdminLockIcon size={18} />
                  <span>Liberação pela administração</span>
                </button>
              )}
            </article>
            ...
            <article className="admin-coach-card">
              ...
              <div className="admin-coach-card-copy">
                <small>VÍNCULO PROFESSOR–ALUNO</small>
                <strong>
                  {selectedStudent.linkedProfessorName ||
                ...
                <p>
                  {selectedStudent.linkedProfessorId
                    ? "O professor principal acompanha e atualiza este treino."
                    : "Defina o responsável antes de montar o programa completo."}
                </p>
              </div>

              {profile?.role === "reception" || profile?.role === "admin" ? (
                <div className="admin-coach-card-controls">
                  <select
                    value={selectedProfessorId}
                    onChange={(event) =>
                      setSelectedProfessorId(event.target.value)
                    }
                    aria-label="Professor principal"
                  >
                    <option value="">Selecione o professor</option>
                    {professors.map((professor) => (
                      <option key={professor.id} value={professor.id}>
                        {professor.fullName} · {professor.activeStudents} aluno
                        {professor.activeStudents === 1 ? "" : "s"}
                      </option>
                    ))}
                  </select>
                  ...
                  <button
                    ...
                    disabled={!selectedProfessorId || linkSaving}
                  >
                ...
                <div className="admin-coach-card-controls is-professor">
                  <span className="admin-coach-other">
                    <AdminWarningIcon size={17} />
                    Outro professor responsável
                  </span>
                </div>
              ) : (
                <div className="admin-coach-card-controls is-professor">
                  <button
                    type="button"
                    className="is-link"
                    onClick={() => void linkSelectedStudent(user?.id ?? "")}
            ...
            {canBuildSelectedTraining ? (
              <article className="admin-training-card">
                <header>
                  <div>
                    ...
                    <p>
                      O programa publicado ficará disponível para o aluno
                      e ligado ao professor principal.
                    </p>
                  </div>
                ...
                <div className="admin-training-actions">
                  ...
                  <button
                    type="button"
                    className="admin-training-action is-complete-action"
                    onClick={() =>
                      navigate(
                  ...
                  >
                    ...
                    <div>
                      <strong>Montagem completa guiada</strong>
                      <small>
                        Objetivo, divisão, dias, exercícios com GIF, carga,
                        descanso e revisão.
                      </small>
                    </div>
                    <AdminChevronIcon />
              ...
              <article className="admin-training-locked">
                ...
                <div>
                  ...
                  <p>
                    Depois da liberação, defina o professor responsável e monte o treino.
                  </p>
                </div>
              </article>
            ) : !selectedHasProfessor ? (
              <article className="admin-training-locked is-link-required">
                <AdminLinkIcon size={26} />
                <div>
                  <strong>Defina o professor principal</strong>
                  <p>
                    O vínculo organiza a responsabilidade pelo aluno e libera a montagem.
                  </p>
                </div>
              </article>
            ) : (
              <article className="admin-training-locked is-other-professor">
                ...
                <div>
                  ...
                  <p>
                    A recepção pode trocar o responsável ou o professor vinculado pode montar o treino.
                  </p>
                </div>
              </article>
            )}
