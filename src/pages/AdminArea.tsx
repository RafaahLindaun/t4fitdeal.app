...
export default function AdminArea() {
  ...
  const authorizeStudent = async () => {
    ...
    try {
      let authorizedStudent = await setWorkoutStudentAccess(
        selectedStudent.id,
        "active",
      );
      let linkNotice = "";
      ...
      setReloadKey((current) => current + 1);
      setToast(
        `Cadastro autorizado com sucesso.${linkNotice}`,
      );
    } catch (error) {
      setToast(
        error instanceof Error
  ...
  const blockStudent = async () => {
    ...
    try {
      const blockedStudent = await setWorkoutStudentAccess(
        selectedStudent.id,
        "blocked",
      );

      refreshStudent(blockedStudent);
      setReloadKey((current) => current + 1);
