...
const GIF_NAME_MAP = {
  ...
  "Puxada (barra/puxador)": "puxada-frente",
  "Puxada frente (puxador)": "puxada-frente",
  "Remada (máquina/curvada)": "remada-baixa",
  ...
  "Agachamento (leve)": "agachamento-leve",
  "Leg press": "leg-press",
  ...
  "Passada (foco glúteo)": "passada-gluteo",
  "Core (dead bug)": "dead-bug",
  "Core (prancha)": "prancha",
  "Desenvolvimento": "desenvolvimento",
  "Desenvolvimento (leve)": "desenvolvimento-leve",
  "Desenvolvimento com halteres": "desenvolvimento-halteres",
  "Elevação lateral": "elevacao-lateral",
  "Posterior (reverse)": "reverse-fly",
  "Encolhimento": "encolhimento",
  "Pallof press": "pallof-press",
  "Abdominal": "abdominal",
  "Supino (leve)": "supino-leve",
  "Remada (leve)": "remada-leve",
  "Posterior (leve)": "posterior-leve",
};
...
function ExerciseGif({ name, size = 48, large = false }) {
  const sources = useMemo(() => gifSourcesForExercise(name), [name]);
  ...
  if (!failed && sources[idx]) {
    return (
      <div style={{ ...S.gifBox, width: size, height: size, borderRadius: large ? 22 : 16 }}>
        <img
          src={sources[idx]}
          alt={name}
          style={S.gifImg}
          ...
          {rows.map((item) => {
            ...
            return (
              <button key={`${item.key}_${item.name}`} style={{ ...S.catalogItem, ...(active ? S.catalogItemOn : null) }} onClick={() => toggle(item.name)} type="button">
                <ExerciseGif name={item.name} size={46} />
                <div style={S.catalogText}>
                  <b>{item.name}</b>
...
const S = {
  ...
  volumeCard: {
    ...
    background: "rgba(255,255,255,.86)",
    border: `1px solid ${BORDER}`,
    boxShadow: "0 10px 26px rgba(15,23,42,.035)",
  },
  ...
  fitdealLogo: {
    ...
    fontSize: 14,
    fontWeight: 980,
    boxShadow: "0 8px 20px rgba(255,106,0,.20)",
  },
};
