...
function gifSources(name) {
  ...
  const names = Array.from(new Set([base, raw, base.replaceAll("-", "_"), raw.replaceAll("-", "_")]));
      ...
      <div style={S.phone}>
        <header style={S.hero}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <h1 style={S.title}>Ajuste simples</h1>
            <p style={S.subtitle}>Escolha o dia, toque em exercício e salve.</p>
          </div>
        </header>
              ...
              <article key={ex.id} style={S.exerciseCard}>
                <button type="button" style={S.exerciseTop} onClick={() => setExpandedExerciseId(expanded ? null : ex.id)}>
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={S.exerciseMeta}>{ex.sets}x • {ex.reps} • {ex.rest}</div>
                  </div>
                    ...
                    <div style={S.methodRow}>
                      {METHOD_OPTIONS.map((m) => (
                        <button
                          key={m.id}
                          type="button"
                          onClick={() => updateExercise(selectedDay.index, ex.id, { method: m.id, pairedWith: m.id === "normal" || m.id === "dropset" ? [] : ex.pairedWith })}
                          style={{ ...S.methodChip, ...(normalizeMethod(ex.method) === m.id ? S.methodChipOn : null) }}
                        >
                          <b>{m.label}</b>
                          <span>{m.hint}</span>
                        </button>
...
function ExercisePicker({ day, currentNames, onPick, onClose }) {
  ...
  const keys = useMemo(() => {
    if (mode === "foco") return day?.group?.pickerKeys || ["peito"];
    return [member];
  }, [mode, member, day?.group?.pickerKeys]);
        ...
        {mode === "membro" ? (
          <div style={S.memberScroller}>
            {Object.keys(EXERCISE_CATALOG).map((key) => (
              <button key={key} type="button" onClick={() => setMember(key)} style={{ ...S.memberChip, ...(member === key ? S.memberChipOn : null) }}>
...
const S = {
  page: { minHeight: "100vh", background: BG, color: TEXT, padding: 12, paddingBottom: 34 },
  phone: { width: "100%", maxWidth: 430, margin: "0 auto" },
  hero: { display: "flex", gap: 12, alignItems: "flex-start", borderRadius: 26, padding: 14, background: "radial-gradient(circle at 92% 0%, rgba(255,106,0,.22), rgba(255,106,0,0) 34%), linear-gradient(135deg, #fff, #fff7ed)", border: `1px solid ${BORDER}`, boxShadow: "0 16px 44px rgba(15,23,42,.075)" },
  heroBack: { width: 44, height: 44, borderRadius: 16, border: `1px solid ${BORDER}`, background: "rgba(255,255,255,.86)", color: TEXT, fontSize: 23, fontWeight: 950, flexShrink: 0 },
  kicker: { color: ORANGE, fontSize: 11, fontWeight: 950, letterSpacing: 1.1, textTransform: "uppercase" },
  title: { margin: "5px 0 0", fontSize: 28, lineHeight: 1, fontWeight: 980, letterSpacing: -1 },
  subtitle: { margin: "8px 0 0", color: MUTED, fontSize: 13, lineHeight: 1.35, fontWeight: 800 },
  card: { marginTop: 12, borderRadius: 24, padding: 13, background: "#fff", border: `1px solid ${BORDER}`, boxShadow: "0 12px 34px rgba(15,23,42,.052)" },
  ...
  dayTab: { border: `1px solid ${BORDER}`, borderRadius: 15, background: "#fff", color: TEXT, padding: 10, fontWeight: 950 },
  dayTabOn: { background: "rgba(255,106,0,.12)", borderColor: "rgba(255,106,0,.28)", color: ORANGE },
  dayHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 },
  sectionTitle: { fontSize: 17, fontWeight: 980, letterSpacing: -0.4 },
  sectionSub: { marginTop: 4, color: MUTED, fontSize: 12, fontWeight: 800 },
  blackBtn: { border: "none", borderRadius: 999, background: BLACK, color: "#fff", padding: "12px 13px", fontSize: 12, fontWeight: 950, whiteSpace: "nowrap" },
  ...
  exerciseMeta: { marginTop: 4, color: MUTED, fontSize: 12, fontWeight: 850 },
  ...
  editGrid: { display: "grid", gridTemplateColumns: "74px 1fr 1fr", gap: 8 },
  label: { display: "grid", gap: 6, color: MUTED, fontSize: 11, fontWeight: 950 },
  input: { width: "100%", boxSizing: "border-box", border: `1px solid ${BORDER}`, borderRadius: 14, background: "#fff", color: TEXT, padding: "10px 8px", fontSize: 13, fontWeight: 900, outline: "none" },
  methodTitle: { color: TEXT, fontSize: 12, fontWeight: 980 },
  methodRow: { display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 },
  methodChip: { border: `1px solid ${BORDER}`, borderRadius: 16, background: "#fff", color: TEXT, padding: 10, display: "grid", gap: 4, textAlign: "left" },
  ...
  sheet: { position: "relative", width: "100%", maxWidth: 430, maxHeight: "86vh", margin: "0 auto", borderRadius: 30, padding: 14, background: "rgba(255,255,255,.98)", border: "1px solid rgba(255,255,255,.60)", boxShadow: "0 30px 90px rgba(15,23,42,.24)", display: "flex", flexDirection: "column", overflow: "hidden" },
  ...
  pickerItem: { width: "100%", border: `1px solid ${BORDER}`, borderRadius: 18, background: "#fff", color: TEXT, padding: 10, display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", textAlign: "left" },
