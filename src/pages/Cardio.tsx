    ...
    <section className="cardio-chart-card" aria-label={`Gráfico ao vivo de ${metric.title}`}>
      ...
      <div className="cardio-chart-wrap">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 7, left: -24, bottom: 0 }}>
            {metric.title === "Pace" ? (
              <>
...
function CardioSummary({
  ...
  const progress = targetSeconds ? clamp(elapsedSeconds / targetSeconds, 0, 1) : 1;
  const radius = 104;
...
export default function Cardio() {
  ...
  const finish = async () => {
    haptic(20);
    const result = await cardioSession.finish();
    if (result.saved) {
      setSaveMessage(
        result.validForRanking
          ? "Cardio salvo no histórico e contabilizado no ranking da academia."
          : cardioSession.elapsedSeconds >= 1800
            ? "Cardio salvo no histórico. O registro permanece disponível para acompanhar sua evolução."
            : "Cardio salvo no histórico. Para pontuar sem musculação no dia, complete pelo menos 30 minutos.",
      );
      if (user) {
        const refreshed = await loadCardioDashboard(user.id);
        setRecentSessions(refreshed.recentSessions);
      }
    } else {
      setSaveMessage("Cardio concluído. O dispositivo não conseguiu confirmar o salvamento no Supabase agora.");
    }
  };
  ...
  if (cardioSession.phase === "finalizado") {
    return (
      <CardioSummary
        ...
        metric={metric}
        saveMessage={saveMessage || "Cardio concluído e registrado."}
    ...
    <div className="cardio-screen">
      ...
      <main className="cardio-page">
        <header className="cardio-topbar">
          <button type="button" className="cardio-top-button" onClick={() => navigate("/menu-teste")} aria-label="Voltar ao menu">
            <WorkoutBackIcon />
          </button>

          <Tabs.Root
            className="cardio-context-tabs"
            value="cardio"
            onValueChange={(value) => {
              if (value === "musculacao") navigate("/treino");
            }}
            aria-label="Alternar entre musculação e cardio"
          >
            <Tabs.List>
              <Tabs.Trigger value="musculacao">Musculação</Tabs.Trigger>
              <Tabs.Trigger value="cardio">Cardio</Tabs.Trigger>
            </Tabs.List>
          </Tabs.Root>

          <button type="button" className="cardio-top-button" onClick={() => setHistoryOpen(true)} aria-label="Abrir histórico de cardio">
            <CardioHistoryIcon />
          </button>
        </header>
