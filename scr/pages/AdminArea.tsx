...
import ProfileMenuItem, { type ProfileMenuItemTone } from "../components/ProfileMenuItem";
...
export default function AdminArea() {
  ...
  const [studentDetailView, setStudentDetailView] = useState<StudentDetailView>(null);
  ...
  const selectedMembershipHealth = selectedStudent
    ? statusMatricula(selectedStudent.membershipValidUntil)
    : "inativa";
  const selectedMembershipDate = selectedStudent?.membershipValidUntil
    ? new Date(`${selectedStudent.membershipValidUntil.slice(0, 10)}T12:00:00`).toLocaleDateString("pt-BR")
    : "não informada";
  const selectedMembershipSubtitle = selectedMembershipHealth === "ativa"
    ? `Ativa até ${selectedMembershipDate}`
    : selectedMembershipHealth === "vencendo"
      ? `Vencendo · ${selectedMembershipDate}`
      : selectedStudent?.membershipValidUntil
        ? `Vencida · ${selectedMembershipDate}`
        : "Matrícula pendente";
  const selectedMembershipTone: ProfileMenuItemTone = selectedMembershipHealth === "ativa"
    ? "success"
    : selectedMembershipHealth === "vencendo"
      ? "warning"
      : "danger";
  const selectedAccessTone: ProfileMenuItemTone = selectedStatus === "active"
    ? "success"
    : selectedStatus === "blocked"
      ? "danger"
      : "warning";
  const selectedCoachSubtitle = selectedLinkedToCurrentProfessor
    ? "Vinculado a você"
    : selectedStudent?.linkedProfessorName || "Sem responsável definido";
  ...
  return (
    <div className={clsx("admin-area-screen", selectedStudent && "is-student-detail-screen")}>
      <div className="admin-area-background" aria-hidden="true">
        <span />
        <i />
      </div>

      <main className={clsx("admin-area-shell", !selectedStudent && "is-dashboard", selectedStudent && "is-student-detail", dashboardView === "library" && !selectedStudent && "is-library-view")}>
        ...
        {!selectedStudent ? (
                      ...
                      {filteredLibraryItems.map((item) => (
                        <SwipeableListItem key={item.id} onDelete={() => setExerciseDelete(item)} deleteLabel="Excluir exercício">
                          <article className="admin-dashboard-resource-card">
                            <span className="admin-library-media"><ExerciseMedia name={item.name} slug={item.slug} mediaUrl={item.mediaUrl} manifestFiles={mediaManifest} compact /></span>
                            <div>
          ...
          <section className="admin-profile-view">
            ...
            <section className="admin-student-action-tiles" aria-label="Ações de treino">
              ...
              <button
                type="button"
                className={clsx("admin-training-action", "is-complete-action", !selectedStudent.hasActiveWorkout && "is-build-urgent")}
                onClick={() => selectedStatus === "active" ? void openCompleteTraining() : setStudentDetailView("access")}
              >
                <span className="is-complete"><AdminSparkIcon /></span>
                <div>
                  <strong>Montar treino agora</strong>
                  <small>{selectedStudent.hasActiveWorkout ? `${selectedStudent.programCode || "Programa ativo"} · atualizar treino` : "Crie a ficha completa com exercícios, GIFs e orientações."}</small>
                </div>
                <AdminChevronIcon />
              </button>
            </section>

            <section className="admin-student-summary-list" aria-label="Detalhes do aluno">
              <ProfileMenuItem
                icon={<AdminClipboardListIcon size={22} />}
                title="Jornada do aluno"
                subtitle={`${selectedJourneyDoneCount}/4 etapas concluídas`}
                tone={selectedJourneyDoneCount === 4 ? "success" : "warning"}
                onClick={() => setStudentDetailView("journey")}
              />
              <ProfileMenuItem
                icon={<AdminBoltIcon size={22} />}
                title="Atividades recentes"
                subtitle={studentActivitiesLoading ? "Carregando registros…" : `${studentActivities.length} registro${studentActivities.length === 1 ? "" : "s"}`}
                tone="default"
                onClick={() => setStudentDetailView("history")}
              />
              <ProfileMenuItem
                icon={<AdminShieldIcon size={22} />}
                title="Acesso ao aplicativo"
                subtitle={statusLabel(selectedStudent.status)}
                tone={selectedAccessTone}
                onClick={() => setStudentDetailView("access")}
              />
              <ProfileMenuItem
                icon={<AdminCalendarIcon size={22} />}
                title="Matrícula e pagamento"
                subtitle={selectedMembershipSubtitle}
                tone={selectedMembershipTone}
                onClick={() => setStudentDetailView("membership")}
              />
              <ProfileMenuItem
                icon={<AdminLinkIcon size={22} />}
                title="Responsável pelo aluno"
                subtitle={selectedCoachSubtitle}
                tone={selectedHasProfessor ? "success" : "warning"}
                onClick={() => setStudentDetailView("coach")}
              />
              <ProfileMenuItem
                icon={<AdminUserIcon size={22} />}
                title="Dados cadastrais"
                subtitle={selectedStudent.email || "E-mail não informado"}
                tone="muted"
                onClick={() => setStudentDetailView("profile")}
              />
            </section>
          </section>
        )}
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "journey"}
          onOpenChange={(open) => setStudentDetailView(open ? "journey" : null)}
          title="Jornada do aluno"
          description={`${selectedJourneyDoneCount}/4 etapas concluídas`}
          className="admin-summary-detail-dialog-v146"
          bodyClassName="admin-summary-detail-body-v146"
        >
          <article className="admin-student-journey is-dialog-content-v146">
            <header>
              <div>
                <small>JORNADA DO ALUNO</small>
                <h2>Próximos passos</h2>
              </div>
              <span>{selectedJourneyDoneCount}/4</span>
            </header>
            <div>
              {[
                {
                  label: "Cadastro",
                  detail: selectedProfileComplete ? "Dados principais preenchidos" : "Complete os dados do perfil",
                  done: selectedProfileComplete,
                  icon: <AdminUserIcon size={18} />,
                },
                {
                  label: "Acesso",
                  detail: selectedStatus === "active" ? "Entrada liberada no aplicativo" : "Autorize o cadastro",
                  done: selectedStatus === "active",
                  icon: <AdminShieldIcon size={18} />,
                },
                {
                  label: "Professor",
                  detail: selectedHasProfessor ? selectedStudent.linkedProfessorName : "Defina o professor principal",
                  done: selectedHasProfessor,
                  icon: <AdminLinkIcon size={18} />,
                },
                {
                  label: "Treino",
                  detail: selectedStudent.hasActiveWorkout ? selectedStudent.programCode || "Programa publicado" : "Monte e publique o programa",
                  done: selectedStudent.hasActiveWorkout,
                  icon: <AdminDumbbellIcon size={18} />,
                },
              ].map((step) => (
                <span key={step.label} className={step.done ? "is-done" : "is-pending"}>
                  <i>{step.icon}</i>
                  <b>{step.label}</b>
                  <small>{step.detail}</small>
                  {step.done ? <AdminCheckIcon size={16} /> : <AdminChevronIcon size={16} />}
                </span>
              ))}
            </div>
          </article>
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "access"}
          onOpenChange={(open) => setStudentDetailView(open ? "access" : null)}
          title="Acesso ao aplicativo"
          description={statusLabel(selectedStudent.status)}
          className="admin-summary-detail-dialog-v146"
          bodyClassName="admin-summary-detail-body-v146"
        >
          <article className="admin-access-card is-dialog-content-v146">
            <span className={`admin-access-icon is-${selectedStatus}`}>
              {selectedStatus === "active" ? (
                <AdminShieldIcon size={27} />
              ) : selectedStatus === "pending" || selectedStatus === "inactive" ? (
                <strong className="admin-access-exclamation" aria-hidden="true">!</strong>
              ) : (
            ...
            <div>
              ...
              <p>{selectedStatus === "active" ? "O aluno consegue entrar normalmente no aplicativo." : "O aluno permanece na tela de espera até a autorização."}</p>
            </div>
            {canApproveAccess ? (
              selectedStatus === "active" ? (
                <button type="button" className="admin-access-action is-block" onClick={() => void blockStudent()} disabled={Boolean(accessSaving)}>
                  <AdminLockIcon size={18} />
                  <span>{accessSaving === "blocked" ? "Bloqueando..." : "Bloquear acesso"}</span>
                </button>
              ) : (
                <button type="button" className="admin-access-action is-authorize" onClick={() => void authorizeStudent()} disabled={Boolean(accessSaving)}>
                  <AdminCheckIcon size={19} />
                  <span>{accessSaving === "active" ? "Autorizando..." : "Autorizar cadastro"}</span>
                </button>
              )
            ) : (
              <button type="button" className="admin-access-action" disabled title="A liberação de entrada é permitida para professor, administração e recepção.">
                <AdminLockIcon size={18} />
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "membership"}
          onOpenChange={(open) => setStudentDetailView(open ? "membership" : null)}
          title="Matrícula e pagamento"
          description={selectedMembershipSubtitle}
          className="admin-summary-detail-dialog-v146"
          bodyClassName="admin-summary-detail-body-v146"
        >
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "coach"}
          onOpenChange={(open) => setStudentDetailView(open ? "coach" : null)}
          title="Responsável pelo aluno"
          description={selectedCoachSubtitle}
          className="admin-summary-detail-dialog-v146"
          bodyClassName="admin-summary-detail-body-v146"
        >
          <article className="admin-coach-card is-dialog-content-v146">
            <span className="admin-coach-card-icon"><AdminLinkIcon size={26} /></span>
            <div className="admin-coach-card-copy">
              <small>RESPONSÁVEL PELO ALUNO</small>
              <strong>{selectedLinkedToCurrentProfessor ? "Vinculado à sua conta" : selectedStudent.linkedProfessorName || "Nenhum responsável definido"}</strong>
              <p>
            ...
            <div className="admin-coach-card-controls is-professor">
              {selectedLinkedToCurrentProfessor ? (
                <>
                  <span className="admin-coach-confirmed"><AdminCheckIcon size={17} />Vinculado a você</span>
                  <button type="button" className="is-unlink" onClick={() => void unlinkSelectedStudent()} disabled={linkSaving}>Remover vínculo</button>
                </>
              ) : (
                <button type="button" className="is-link" onClick={() => void linkSelectedStudent()} disabled={!user?.id || linkSaving}>
                  <AdminLinkIcon size={18} />
                  {linkSaving ? "Vinculando..." : "Vincular a mim"}
                </button>
              )}
            </div>
          </article>
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "profile"}
          onOpenChange={(open) => setStudentDetailView(open ? "profile" : null)}
          title="Dados cadastrais"
          description={selectedStudent.fullName}
          className="admin-profile-details-dialog-v143"
          bodyClassName="admin-profile-details-body-v143"
        >
          <div className="admin-profile-details-grid-v143">
            <span><small>Nome completo</small><strong>{selectedStudent.fullName || "Não informado"}</strong></span>
            <span><small>Matrícula</small><strong>{selectedStudent.registrationCode || "Não informada"}</strong></span>
            <span><small>E-mail</small><strong>{selectedStudent.email || "Não informado"}</strong></span>
            <span><small>Telefone</small><strong>{selectedStudent.phone || "Não informado"}</strong></span>
            <span><small>CPF</small><strong>{formatStudentDocument(selectedStudent.cpf) || "Não informado"}</strong></span>
            <span><small>RG</small><strong>{selectedStudent.rg || "Não informado"}</strong></span>
            <span><small>Gympass</small><strong>{selectedStudent.gympassNumber || "Não informado"}</strong></span>
            <span><small>Matrícula válida até</small><strong>{selectedStudent.membershipValidUntil ? new Date(`${selectedStudent.membershipValidUntil}T12:00:00`).toLocaleDateString("pt-BR") : "Não informada"}</strong></span>
            <span><small>Nascimento</small><strong>{selectedStudent.birthDate ? new Date(`${selectedStudent.birthDate.slice(0, 10)}T12:00:00`).toLocaleDateString("pt-BR") : "Não informado"}</strong></span>
            <span><small>Objetivo</small><strong>{selectedStudent.objective || "Não informado"}</strong></span>
          </div>
          <div className="admin-profile-details-actions-v143">
            <button type="button" className="is-edit" onClick={() => { setStudentDetailView(null); setEditForm(studentToForm(selectedStudent)); setEditing(true); }}><AdminEditIcon size={17} /> Editar informações</button>
            <button type="button" className="is-danger" onClick={() => { setStudentDetailView(null); setDeleteConfirmation(""); setDeleteOpen(true); }}><AdminCloseIcon size={17} /> Apagar conta definitivamente</button>
          </div>
        </ResponsiveDialog>
      ) : null}
      ...
      {selectedStudent ? (
        <ResponsiveDialog
          open={studentDetailView === "history"}
          onOpenChange={(open) => setStudentDetailView(open ? "history" : null)}
          title="Histórico de atividades"
          description={`${selectedStudent.fullName} · ${studentActivities.length} registro${studentActivities.length === 1 ? "" : "s"}`}
          className="admin-history-dialog-v142"
          bodyClassName="admin-history-dialog-body-v142"
        >
          <div className="admin-history-dialog-list-v142">
            {studentActivities.map((activity) => (
              <article key={`history-${activity.id}`} className="admin-history-dialog-item-v142">
                <i className={`is-${activity.kind}`}>
                  {activity.kind === "cardio" ? <AdminBoltIcon size={18} /> : <AdminDumbbellIcon size={18} />}
                </i>
                <div>
                  <strong>{activity.title || (activity.kind === "cardio" ? "Cardio" : "Treino")}</strong>
                  <small>{formatStudentActivityDate(activity.performedAt)}</small>
                </div>
                <dl>
                  <div><dt>Duração</dt><dd>{formatStudentActivityDuration(activity.durationSeconds)}</dd></div>
                  {activity.kind === "workout" ? <div><dt>Conclusão</dt><dd>{Math.round(activity.completionPercentage)}%</dd></div> : null}
                  {activity.exerciseCount ? <div><dt>Exercícios</dt><dd>{activity.exerciseCount}</dd></div> : null}
                  {activity.totalSets ? <div><dt>Séries</dt><dd>{activity.completedSets ?? 0}/{activity.totalSets}</dd></div> : null}
                  {activity.calories > 0 ? <div><dt>Calorias</dt><dd>{Math.round(activity.calories)} kcal</dd></div> : null}
                </dl>
                {activity.validForRanking ? <b title="Contou para o ranking"><AdminCheckIcon size={14} /></b> : null}
              </article>
            ))}
            {!studentActivities.length ? <EmptyState icon={<AdminDumbbellIcon size={26} />} title="Nenhum registro" description="Treinos e cardios concluídos aparecerão aqui." /> : null}
          </div>
        </ResponsiveDialog>
      ) : null}
      ...
      {editing && editForm && selectedStudent ? (
        <ResponsiveDialog
          open={editing}
          onOpenChange={(open) => { if (!saving) setEditing(open); }}
          title="Editar perfil"
          description={selectedStudent.fullName}
          className="admin-edit-dialog-v142"
          bodyClassName="admin-edit-dialog-body-v142"
        >
          <form className="admin-edit-dialog-form-v142" onSubmit={submitEdit}>
