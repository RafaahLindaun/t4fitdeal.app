          ...
          <section className="admin-profile-view">
            ...
            <article className="admin-student-history-card">
              <header>
                ...
                <div className="admin-student-history-head-actions">
                  <span>{studentActivities.length}</span>
                  {studentActivities.length > 0 ? (
                    <button type="button" onClick={() => setHistoryOpen(true)}>Ver todos</button>
                  ) : null}
                </div>
              </header>

              {studentActivitiesLoading ? (
                <div className="admin-student-history-skeleton" aria-label="Carregando registros" aria-busy="true">
                  {Array.from({ length: 4 }, (_, index) => <span key={index}><i /><b /><small /></span>)}
                </div>
            ...
            <article className="admin-profile-data admin-profile-data-summary">
              <header>
                <div>
                  <small>DADOS CADASTRAIS</small>
                  <h2>Informações do aluno</h2>
                </div>
                <button type="button" onClick={() => setProfileDetailsOpen(true)}>Ver detalhes</button>
              </header>
      ...
      {selectedStudent ? (
        ...
        >
          ...
          <div className="admin-profile-details-actions-v143">
            <button type="button" className="is-edit" onClick={() => { setProfileDetailsOpen(false); setEditForm(studentToForm(selectedStudent)); setEditing(true); }}><AdminEditIcon size={17} /> Editar informações</button>
            <button type="button" className="is-danger" onClick={() => { setProfileDetailsOpen(false); setDeleteConfirmation(""); setDeleteOpen(true); }}><AdminCloseIcon size={17} /> Apagar conta definitivamente</button>
          </div>
        </ResponsiveDialog>
      ) : null}
