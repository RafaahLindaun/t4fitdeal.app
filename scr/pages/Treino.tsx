...
export default function Treino() {
  ...
  const handoffSessionId = useMemo(
  ...
  // Build 1.4.4: treino pronto = execução ativa. O AppShell entra em modo foco
  // e restaura a navegação automaticamente ao sair/desmontar esta tela.
  ...
  useEffect(() => {
    if (!user?.id || !handoffSessionId || viewState !== "ready" || !exercises.length) return;
    if (restoredHandoffRef.current === handoffSessionId) return;

    let cancelled = false;
    void loadWorkoutSessionProgress(user.id, handoffSessionId).then((progress) => {
      if (cancelled || !progress) return;
      restoredHandoffRef.current = handoffSessionId;
      setSession({ id: progress.id, local: false });
      const startedAt = new Date(progress.startedAt).getTime();
      setSessionStartedAt(Number.isFinite(startedAt) ? startedAt : Date.now());

      const completed: Record<string, boolean> = {};
      const restoredDrafts: Record<string, SetDraft> = {};
      for (const set of progress.completedSets) {
        const key = setDraftKey(set.exerciseId, set.setNumber);
        completed[key] = true;
        restoredDrafts[key] = { loadKg: set.loadKg, reps: set.reps };
      }
      setCompletedSets(completed);
      setSetDrafts((previous) => ({ ...previous, ...restoredDrafts }));

      const nextCursor: Record<string, number> = {};
      for (const exercise of exercises) {
        let nextSet = 1;
        while (nextSet <= exercise.sets && completed[setDraftKey(exercise.id, nextSet)]) nextSet += 1;
        nextCursor[exercise.id] = Math.min(exercise.sets, nextSet);
      }
      setSetCursor(nextCursor);

      const nextExerciseIndex = exercises.findIndex((exercise) =>
        Array.from({ length: exercise.sets }, (_, index) => !completed[setDraftKey(exercise.id, index + 1)]).some(Boolean),
      );
      setExerciseIndex(nextExerciseIndex >= 0 ? nextExerciseIndex : Math.max(0, exercises.length - 1));
      setToast(progress.completedSets.length ? "Sessão retomada do ponto em que você parou." : "Sessão do celular preparada.");
    });
  ...
  useEffect(() => {
    const syncScale = () => {
      ...
      const renderedScreenHeight = screen?.getBoundingClientRect().height ?? height;
      const renderedScreenWidth = screen?.getBoundingClientRect().width ?? width;
      const mobile = width < 1024;
      // Build 1.4.1: no mobile a área do treino já termina antes do BottomNav.
      // Escalar usando a área realmente renderizada evita o CTA ficar atrás da navegação
      // ou do chrome variável do Safari/Chrome. A margem extra cobre a altura real do stage.
      const availableHeight = Math.max(320, Math.min(height, renderedScreenHeight) - 10);
      const availableWidth = Math.max(280, Math.min(width, renderedScreenWidth) - 10);
      const referenceHeight = mobile ? MOBILE_SAFE_ARTBOARD_HEIGHT : ARTBOARD_HEIGHT;
      ...
      document.documentElement.style.setProperty(
        ...
        String(Math.max(mobile ? 0.34 : 0.55, scale)),
      );
    };
  ...
  useEffect(
    () => () => {
      ...
      if (restConfirmationTimer.current) {
        window.clearTimeout(restConfirmationTimer.current);
      }
  ...
  const finishWorkout = async (
    ...
    try {
      ...
      if (result.saved) {
        await recordWorkoutSessionCompletion({
          alunoId: user.id,
          sessionId: activeSession.id,
          completionPercentage: percentage,
          durationSeconds,
        });
        if (percentage >= 100) markWorkoutCompletionTransition(user.id);
        await queryClient.invalidateQueries({ queryKey: ["treino-status", user.id] });
        window.dispatchEvent(new CustomEvent("accqua:treino-status-invalidated", { detail: { userId: user.id } }));
  ...
  const showRestConfirmation = (message: string) => {
    if (restConfirmationTimer.current) window.clearTimeout(restConfirmationTimer.current);
    setRestConfirmation(message);
    if (reduceMotion) { setRestConfirmation(""); return; }
    restConfirmationTimer.current = window.setTimeout(() => setRestConfirmation(""), 1350);
  };
    ...
    return (
      <div className="workout-screen">
        ...
        <main className="workout-empty-shell">
          <PageHeader className="workout-empty-header" ariaLabel="Cabeçalho do treino"
            left={<button type="button" className="workout-round-button" onClick={() => navigate("/menu-teste")} aria-label="Voltar"><WorkoutBackIcon /></button>}
            center={<div className="workout-header-brand"><span className="workout-header-logo-crop"><img src="/accqua-logo-header.png" alt="Accqua Sports" /></span><i/><strong>MEU TREINO</strong></div>}
            right={<div className="workout-topbar-actions"><button type="button" className="workout-mode-switch-button" onClick={() => navigate("/cardio")} aria-label="Ir para cardio" title="Ir para cardio"><CardioSwapIcon/><small>CARDIO</small></button><button type="button" className="workout-round-button calendar" onClick={() => setCalendarOpen(true)} aria-label="Calendário"><WorkoutCalendarIcon/></button></div>}
          />
  ...
  return (
    <div className="workout-screen">
      ...
      <main className="workout-artboard">
        <PageHeader className="workout-topbar" ariaLabel="Cabeçalho do treino em andamento"
          left={<motion.button type="button" className="workout-round-button" onClick={() => setExitOpen(true)} whileTap={{ scale: 0.9 }} aria-label="Voltar"><WorkoutBackIcon /></motion.button>}
          center={<div className="workout-header-brand"><span className="workout-header-logo-crop"><img src="/accqua-logo-header.png" alt="Accqua Sports" /></span><i/><motion.button type="button" className="workout-plan-title-button" onClick={() => setTitleExpanded((value) => !value)} whileTap={{ scale: 0.97 }} aria-expanded={titleExpanded} title={plan.name}><strong>{planDisplayName(plan.name)}</strong></motion.button>{titleExpanded ? <div className="workout-plan-title-expanded" role="status">{planDisplayName(plan.name)}</div> : null}</div>}
              ...
              {restRemaining > 0 ? (
                <>
                  ...
                  <AnimatePresence mode="wait" initial={false}>
                    {restConfirmation ? (
                      <motion.div key="rest-confirmation" className="workout-rest-confirmation" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }}>
                        <span><WorkoutCheckIcon size={34} /></span>
                        <strong>{restConfirmation}</strong>
                      </motion.div>
                    ) : (
