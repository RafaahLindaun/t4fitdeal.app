...
import ProfileTrainingPartners from "../components/ProfileTrainingPartners";
...
export default function Profile() {
  const navigate = useNavigate(); const queryClient = useQueryClient(); const { user, profile, loading, landingPath, signOut, refreshProfile } = useAuth();
  ...
  const partnerCountQuery = useQuery({ queryKey: ["training-partners", "count", user?.id], queryFn: loadMyTrainingPartnerCount, enabled: Boolean(user?.id && profile?.role === "student"), staleTime: 20_000, refetchOnWindowFocus: false });
  ...
  const [view, setView] = useState<ProfileView>(() => { const query = new URLSearchParams(window.location.search).get("view"); return query === "settings" ? "settings" : "main"; });
  const [dashboard, setDashboard] = useState<ProfileDashboard>(emptyDashboard); const [loadingDashboard, setLoadingDashboard] = useState(true); const [message, setMessage] = useState("");
  ...
  const savePersonal = async (event: FormEvent) => { event.preventDefault(); if (!personalForm.fullName.trim()) { setMessage("Informe seu nome completo."); return; } setSaving(true); const result = await savePersonalProfile(personalForm); setSaving(false); if (result.error) { setMessage("Não foi possível salvar seus dados."); return; } await refreshProfile(); await reload(); setMessage("Dados pessoais atualizados."); };
  const updatePreferences = async (key: Exclude<keyof ProfilePreferences, "classReminderMinutes">, value: boolean) => { const next = { ...dashboard.preferences, [key]: value }; setDashboard((current) => ({ ...current, preferences: next })); if (["workoutNotifications", "classNotifications", "activityReminders", "newsNotifications"].includes(key) && value && "Notification" in window) { try { await Notification.requestPermission(); } catch {} } cacheFeedbackPreferences(user.id, { soundEnabled: next.soundEnabled, vibrationEnabled: next.vibrationEnabled }); if (key === "soundEnabled" && value) playAccquaChime(user.id, "success"); if (key === "vibrationEnabled" && value) performHaptic(user.id, [45, 25, 45]); const result = await saveProfilePreferences(user.id, next); setMessage(result.error ? "Não foi possível salvar a preferência." : "Preferência salva."); };
  const savePassword = async (event: FormEvent) => { event.preventDefault(); if (passwordForm.password.length < 8) { setMessage("A nova senha precisa ter pelo menos 8 caracteres."); return; } if (passwordForm.password !== passwordForm.confirmation) { setMessage("As senhas digitadas não são iguais."); return; } setSaving(true); const result = await changeMyPassword(passwordForm.password); setSaving(false); if (result.error) { setMessage("Não foi possível alterar a senha."); return; } setPasswordForm({ password: "", confirmation: "" }); setMessage("Senha alterada com segurança."); };
  const logout = async () => { setLoggingOut(true); await signOut(); navigate("/login", { replace: true }); };
  ...
  return <div className="accqua-profile-screen">
    ...
    <main className="accqua-profile-shell">
      ...
      <section className={`accqua-profile-content ${view === "main" ? "is-main-view" : "is-subview"}`} ref={contentRef}>
        ...
        {view === "workouts" ? <section className="profile-subview">{dashboard.activeWorkout ? <article className="profile-active-plan"><span><ProfileIcon name="dumbbell" size={29} /></span><div><small>PROGRAMA ATIVO</small><h2>{dashboard.activeWorkout.programName}</h2><p>Divisão {dashboard.activeWorkout.splitType || "personalizada"} · {dashboard.activeWorkout.routines} ficha{dashboard.activeWorkout.routines === 1 ? "" : "s"}</p>{dashboard.activeWorkout.reviewAt ? <i>Revisão em {formatDate(dashboard.activeWorkout.reviewAt)}</i> : null}</div></article> : <div className="accqua-profile-empty"><span><ProfileIcon name="dumbbell" size={31} /></span><strong>Nenhum treino ativo</strong><p>Quando seu professor publicar uma ficha, ela aparecerá aqui.</p></div>}<div className="profile-section-heading"><div><small>REGISTROS</small><h3>Treinos concluídos</h3></div><strong>{workoutHistory.length}</strong></div><ActivityList activities={workoutHistory} /></section> : null}
        {view === "classes" ? <section className="profile-subview"><div className="profile-section-heading"><div><small>AGENDA</small><h3>Próximas aulas</h3></div><strong>{upcomingClasses.length}</strong></div><ActivityList activities={upcomingClasses} /><div className="profile-section-heading is-spaced"><div><small>HISTÓRICO</small><h3>Aulas realizadas</h3></div><strong>{pastClasses.length}</strong></div><ActivityList activities={pastClasses} /></section> : null}
        {view === "notifications" ? <section className="profile-subview"><div className="profile-info-banner"><ProfileIcon name="bell" size={25} /><div><strong>Alertas do seu treino</strong><p>Escolha quais avisos deseja receber no aplicativo e no celular.</p></div></div>{notificationPermission !== "granted" && notificationPermission !== "unsupported" ? <button className="profile-notification-permission" type="button" onClick={() => void enableDeviceNotifications()}>Ativar notificações do celular</button> : null}<div className="profile-toggle-card"><ProfileToggle title="Treinos e atualizações" subtitle="Nova ficha, alteração e lembrete de treino" checked={dashboard.preferences.workoutNotifications} onChange={(value) => void updatePreferences("workoutNotifications", value)} /><ProfileToggle title="Aulas e horários" subtitle="Reservas, mudanças e lembretes de aula" checked={dashboard.preferences.classNotifications} onChange={(value) => void updatePreferences("classNotifications", value)} /><label className="profile-reminder-select"><span><strong>Lembrete antes da aula</strong><small>Para aulas que você já reservou</small></span><select value={dashboard.preferences.classReminderMinutes} disabled={!dashboard.preferences.classNotifications} onChange={(event) => { const next = { ...dashboard.preferences, classReminderMinutes: Number(event.target.value) }; setDashboard((current) => ({ ...current, preferences: next })); cacheFeedbackPreferences(user.id, { classReminderMinutes: next.classReminderMinutes }); void saveProfilePreferences(user.id, next).then((result) => setMessage(result.error ? "Não foi possível salvar a preferência." : "Preferência salva.")); }}><option value={30}>30 min antes</option><option value={60}>1h antes</option><option value={120}>2h antes</option><option value={240}>4h antes</option></select></label><ProfileToggle title="Lembrete de atividade" subtitle="Ajuda para manter sua frequência" checked={dashboard.preferences.activityReminders} onChange={(value) => void updatePreferences("activityReminders", value)} /><ProfileToggle title="Novidades da ACCQUA" subtitle="Comunicados importantes da academia" checked={dashboard.preferences.newsNotifications} onChange={(value) => void updatePreferences("newsNotifications", value)} /></div></section> : null}
        {view === "settings" ? <section className="profile-subview"><div className="profile-toggle-card"><ProfileToggle title="Aparecer no ranking" subtitle="Mostra seu primeiro nome e foto" checked={personalForm.showInRanking} onChange={async (value) => { const next = { ...personalForm, showInRanking: value }; setPersonalForm(next); const result = await savePersonalProfile(next); setMessage(result.error ? "Não foi possível salvar." : "Configuração salva."); }} /><ProfileToggle title="Sons do aplicativo" subtitle="Avisos sonoros durante as atividades" checked={dashboard.preferences.soundEnabled} onChange={(value) => void updatePreferences("soundEnabled", value)} /><ProfileToggle title="Vibração" subtitle="Feedback sutil nos controles" checked={dashboard.preferences.vibrationEnabled} onChange={(value) => void updatePreferences("vibrationEnabled", value)} /><ProfileToggle title="Repouso obrigatório entre séries" subtitle="Desative para liberar a opção de pular o descanso" checked={dashboard.preferences.restRequired} onChange={(value) => void updatePreferences("restRequired", value)} /></div><div className="profile-settings-note"><ProfileIcon name="shield" size={23} /><div><strong>Privacidade protegida</strong><p>CPF, telefone e e-mail continuam privados. No ranking aparecem somente o primeiro nome e a foto escolhida.</p></div></div></section> : null}
        {view === "security" ? <section className="profile-subview"><div className="profile-security-card"><span><ProfileIcon name="shield" size={31} /></span><div><strong>Sua conta está protegida</strong><p>Altere sua senha sempre que achar necessário.</p></div></div><form className="profile-form" onSubmit={savePassword}><div className="profile-form-card"><label>Nova senha<input type="password" autoComplete="new-password" value={passwordForm.password} onChange={(event) => setPasswordForm({ ...passwordForm, password: event.target.value })} placeholder="Mínimo de 8 caracteres" /></label><label>Confirmar nova senha<input type="password" autoComplete="new-password" value={passwordForm.confirmation} onChange={(event) => setPasswordForm({ ...passwordForm, confirmation: event.target.value })} placeholder="Digite novamente" /></label></div><button className="profile-primary-button" type="submit" disabled={saving}>{saving ? "Alterando..." : "Alterar senha"}</button></form></section> : null}
        {view === "support" ? <section className="profile-subview"><div className="profile-support-hero"><span><ProfileIcon name="help" size={34} /></span><h2>Como podemos ajudar?</h2><p>Encontre orientações rápidas ou fale com a recepção da academia.</p></div><div className="profile-menu-card"><a className="profile-menu-item" href="tel:+551147181730"><span className="profile-menu-icon"><ProfileIcon name="phone" size={22} /></span><div><strong>Ligar para a recepção</strong><p>(11) 4718-1730</p></div><ProfileIcon name="chevron" size={19} /></a><button className="profile-menu-item" type="button" onClick={() => setMessage("Leve esta tela até a recepção para receber ajuda.")}><span className="profile-menu-icon"><ProfileIcon name="user" size={22} /></span><div><strong>Atendimento presencial</strong><p>Ajuda com cadastro, acesso e matrícula</p></div><ProfileIcon name="chevron" size={19} /></button><button className="profile-menu-item" type="button" onClick={() => setMessage("Atualize a página e tente novamente. Se continuar, fale com a recepção.")}><span className="profile-menu-icon"><ProfileIcon name="settings" size={22} /></span><div><strong>Problemas no aplicativo</strong><p>Orientações para acesso e carregamento</p></div><ProfileIcon name="chevron" size={19} /></button></div></section> : null}
      </section>
...
function ProfileToggle({ title, subtitle, checked, onChange }: { title: string; subtitle: string; checked: boolean; onChange: (value: boolean) => void; }) {
  return <label className="profile-toggle-row"><div><strong>{title}</strong><p>{subtitle}</p></div><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span className="profile-toggle-control"><i /></span></label>;
}
