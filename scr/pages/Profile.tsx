...
export default function Profile() {
  ...
  useEffect(() => {
    if (view !== "main" || loadingDashboard) return;
    const section = new URLSearchParams(window.location.search).get("section");
    if (section !== "reservas") return;
    const frame = window.requestAnimationFrame(() => {
      document.getElementById("profile-reservations")?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
    return () => window.cancelAnimationFrame(frame);
  }, [loadingDashboard, view]);
            ...
            <>
              ...
              <section className="profile-menu-section is-account-menu">
                ...
                <div className="profile-menu-card">
                  <ProfileMenuItem icon={<ProfileIcon name="user" size={22} />}  title="Dados pessoais" subtitle="Nome, contato, CPF e objetivo" onClick={() => openView("personal")} />
              ...
              <section className="profile-menu-section is-preferences-menu">
                ...
                <div className="profile-menu-card">
                  <ProfileMenuItem icon={<ProfileIcon name="bell" size={22} />}  title="Notificações" subtitle="Lembretes e avisos do aplicativo" onClick={() => openView("notifications")} />
                  <ProfileMenuItem icon={<ProfileIcon name="settings" size={22} />}  title="Configurações" subtitle="Ranking, som e vibração" onClick={() => openView("settings")} />
                  <ProfileMenuItem icon={<ProfileIcon name="lock" size={22} />}  title="Segurança" subtitle="Senha e acesso à conta" onClick={() => openView("security")} />
                  <ProfileMenuItem icon={<ProfileIcon name="help" size={22} />}  title="Ajuda e suporte" subtitle="Orientações e recepção" onClick={() => openView("support")} />
                </div>
              </section>

              <section id="profile-reservations" className="profile-menu-section profile-reservations-section">
