...
export default function AdminWorkoutBuilder() {
  ...
  useEffect(() => {
    ...
    const loadBuilder = async () => {
      try {
        const mediaManifestResult = await loadExerciseMediaManifest();
        const [
          studentResult,
          libraryResult,
          templateResult,
          ...
          loadSyncedExerciseLibrary(mediaManifestResult),
    ...
    () => [
      {
        ...
        detail: linkedProfessorReady
          ...
          : "No perfil do aluno, toque em “Vincular a mim” antes de salvar.",
      ...
      },
      {
        ...
        detail: programName.trim()
          ? programName
          : "Na etapa Programa, informe o nome do programa.",
      ...
      },
      {
        ...
        detail: routinesConfigured
          ...
          : "Na etapa Dias, dê um nome e escolha os dias de cada rotina.",
      ...
      },
      {
        ...
        detail: hasTrainingContent
          ...
          : totalExercises > 0
            ? "Na etapa Exercícios, adicione pelo menos 1 exercício em cada rotina."
            : "Na etapa Exercícios, adicione exercícios; ou use Cardio se o programa for somente aeróbico.",
      ...
      },
      {
        ...
        detail: exerciseParametersValid
          ...
          : "Na etapa Exercícios, ajuste séries, repetições e descanso.",
      ...
      },
      {
        ...
        detail: reviewAt
          ...
          : "Na etapa Programa, defina uma data se quiser programar a revisão.",
  ...
  const stepPresentation = useMemo(() => {
    return BUILDER_STEPS.reduce(
      (accumulator, step) => {
        if (step.key === "cardio" && !cardio.enabled) {
          accumulator[step.key] = { state: "optional", label: "Opcional" };
          return accumulator;
        }

        const requiredForStep = readinessItems.filter(
          (item) => item.required && item.step === step.key,
        );
        const isComplete =
          requiredForStep.length > 0 && requiredForStep.every((item) => item.done);

        accumulator[step.key] = {
          state: isComplete ? "complete" : "pending",
          label: isComplete ? "Concluído" : "Pendente",
        };
        return accumulator;
      },
      {} as Record<BuilderStep, { state: "complete" | "pending" | "optional"; label: string }>,
    );
  }, [cardio.enabled, readinessItems]);
          ...
          {BUILDER_STEPS.map((step) => {
            ...
            return (
              <button
                ...
                key={step.key}
                className={clsx(
                  mobileStep === step.key && "is-active",
                  `is-${presentation.state}`,
                )}
                aria-pressed={mobileStep === step.key}
                onClick={() => showBuilderSection(step.key)}
                title={`${step.label}: ${presentation.label}`}
              >
                <span>
                  {presentation.state === "complete" ? (
                    <AdminCheckIcon size={14} />
                  ) : (
                    step.number
                  )}
                </span>
                  ...
                  >
                    ...
                    >
                      <span className="admin-builder-selected-gif">
                        <ExerciseMedia name={exercise.name} slug={exercise.slug} mediaUrl={exercise.mediaUrl} manifestFiles={mediaManifest} compact />
                        <b>{index + 1}</b>
                      </span>
                    ...
                    {expanded ? (
                      <div className="admin-builder-exercise-editor">
                        <div className="admin-builder-exercise-gif-editor">
                          <ExerciseMedia name={exercise.name} slug={exercise.slug} mediaUrl={exercise.mediaUrl} manifestFiles={mediaManifest} />
                          <label>
            ...
            {filteredLibrary.map((item) => (
              <article key={item.slug}>
                <span className="admin-builder-library-gif">
                  <ExerciseMedia name={item.name} slug={item.slug} mediaUrl={item.mediaUrl} manifestFiles={mediaManifest} compact />
                </span>
                <div>
                  <strong>{item.name}</strong>
                  <small>
                ...
                <button
                  type="button"
                  className="admin-builder-library-add-item"
