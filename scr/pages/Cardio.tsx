...
function PaceChart({ samples, metric }: { samples: CardioSample[]; metric: MetricDescriptor }) {
  ...
  return (
    <section className="cardio-chart-card" aria-label={`Gráfico ao vivo de ${metric.title}`}>
      <header><div><span>RITMO AO VIVO</span><strong>{metric.title}</strong></div><small>últimos 3 min</small></header>
      <div className="cardio-chart-wrap">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 7, left: -24, bottom: 0 }}>
            <defs><linearGradient id="cardio-live-area" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--cardio-accent)" stopOpacity={0.26} /><stop offset="100%" stopColor="var(--cardio-accent)" stopOpacity={0.015} /></linearGradient></defs>
            {metric.title === "Pace" ? <><ReferenceArea y1={metric.min} y2={zone1} fill="var(--cardio-zone-hard)" fillOpacity={0.09} /><ReferenceArea y1={zone1} y2={zone2} fill="var(--cardio-zone-medium)" fillOpacity={0.12} /><ReferenceArea y1={zone2} y2={metric.max} fill="var(--cardio-zone-light)" fillOpacity={0.18} /></> : <><ReferenceArea y1={metric.min} y2={zone1} fill="var(--cardio-zone-light)" fillOpacity={0.18} /><ReferenceArea y1={zone1} y2={zone2} fill="var(--cardio-zone-medium)" fillOpacity={0.12} /><ReferenceArea y1={zone2} y2={metric.max} fill="var(--cardio-zone-hard)" fillOpacity={0.09} /></>}
            <XAxis dataKey="label" hide /><YAxis domain={[metric.min, metric.max]} hide />
            <Tooltip cursor={{ stroke: "rgba(255,255,255,.12)", strokeWidth: 1 }} content={({ active, payload, label }) => { if (!active || !payload?.length) return null; const raw = Number(payload[0]?.value ?? 0); return <div className="cardio-chart-tooltip"><span>{label}</span><strong>{metric.formatter(raw)} {metric.unit}</strong></div>; }} />
            <Area type="monotone" dataKey="value" stroke="var(--cardio-accent)" strokeWidth={3} fill="url(#cardio-live-area)" dot={false} activeDot={{ r: 4, fill: "var(--cardio-accent-hot)", strokeWidth: 0 }} isAnimationActive={false} connectNulls />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <footer aria-label="Zonas estimadas de esforço"><span><i className="zone-light" />Leve</span><span><i className="zone-medium" />Moderado</span><span><i className="zone-hard" />Intenso</span></footer>
    </section>
  );
}

function SummaryMetric({ label, value, formatter, reducedMotion }: { label: string; value: number; formatter: (value: number) => string; reducedMotion: boolean }) {
  return <article><span>{label}</span><strong><AnimatedMetric value={value} formatter={formatter} reducedMotion={reducedMotion} /></strong></article>;
}

function CardioSummary({ activity, elapsedSeconds, targetSeconds, distanceMeters, calories, metric, syncStatus, validForRanking, reducedMotion, onRetrySync, onAgain, onMenu }: {
  activity: CardioActivity; elapsedSeconds: number; targetSeconds: number; distanceMeters: number; calories: number; metric: MetricDescriptor;
  syncStatus: "idle" | "pending" | "synced" | "failed"; validForRanking: boolean; reducedMotion: boolean; onRetrySync: () => void; onAgain: () => void; onMenu: () => void;
}) {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      confetti({ particleCount: 4, spread: 52, startVelocity: 19, gravity: 0.8, scalar: 0.7, origin: { x: 0.5, y: 0.68 }, colors: ["#ffd11e", "#ff9b2f", "#ffffff"], disableForReducedMotion: true });
...
export default function Cardio() {
  ...
  const selectedPrescription = useMemo(() => prescriptions.find((prescription) => prescription.activityType === activity && prescription.isActive) ?? null, [activity, prescriptions]);
  ...
  const applyActivity = useCallback((nextActivity: CardioActivity, prescription?: CardioPrescription | null) => {
    ...
    setActivity(nextActivity); setTiming(prescription?.timing ?? "anytime"); setTargetMinutes(clamp(prescription?.targetDurationMinutes || 30, 5, 120)); setSpeedKmh(Number(nextSpeed.toFixed(1))); setPaceSeconds(speedDriven ? paceFromSpeed(nextSpeed) : prescription?.targetPaceSeconds || defaults.pace); setCadenceRpm(defaults.cadence); setPoolLengthMeters(25); setLaps(0);
  }, []);

  useEffect(() => { if (isSpeedBased) setPaceSeconds(paceFromSpeed(speedKmh)); }, [isSpeedBased, speedKmh]);
  const targetSeconds = targetMinutes * 60;
  const calculatedSpeed = isSpeedBased || activity === "spinning" ? speedKmh : 0;
  const metricsForElapsed = useCallback((seconds: number) => {
    const distanceMeters = activity === "swim" ? laps * poolLengthMeters : activity === "rowing" ? paceSeconds > 0 ? Math.round((seconds / paceSeconds) * 500) : 0 : activity === "stairs" ? Math.round(cadenceRpm * (seconds / 60)) : Math.round(calculatedSpeed * (seconds / 3600) * 1000);
  ...
  const buildSnapshot = useCallback((seconds: number, status: CardioSessionStatus) => { const metrics = metricsForElapsed(seconds); return { elapsedSeconds: seconds, distanceMeters: metrics.distanceMeters, averagePaceSeconds: paceSeconds, averageSpeedKmh: calculatedSpeed, cadenceRpm, laps, calories: metrics.calories, status }; }, [calculatedSpeed, cadenceRpm, laps, metricsForElapsed, paceSeconds]);
  ...
  const metric = useMemo<MetricDescriptor>(() => {
    if (activity === "stairs") return { title: "Ritmo", value: cadenceRpm, unit: "degraus/min", formatter: (value) => String(Math.round(value)), min: 20, max: 180 };
    if (activity === "rowing") return { title: "Pace", value: paceSeconds, unit: "min/500m", formatter: formatPace, min: 80, max: 300 };
    if (activity === "swim") return { title: "Pace", value: paceSeconds, unit: "min/100m", formatter: formatPace, min: 45, max: 300 };
    return { title: "Velocidade", value: speedKmh, unit: "km/h", formatter: (value) => value.toFixed(1), min: intensityConfig.min, max: intensityConfig.max };
  }, [activity, cadenceRpm, intensityConfig.max, intensityConfig.min, paceSeconds, speedKmh]);

  const startConfig = useMemo(() => ({ userId: user?.id ?? "", prescriptionId: selectedPrescription?.id ?? null, activityType: activity, timing, targetDurationSeconds: targetSeconds, targetSnapshot: { duration_minutes: targetMinutes, pace_seconds: paceSeconds, speed_kmh: calculatedSpeed, cadence_rpm: cadenceRpm, pool_length_meters: poolLengthMeters, mode_label: getActivityOption(activity).label }, source: selectedPrescription ? "professor" as const : "free" as const }), [activity, calculatedSpeed, cadenceRpm, paceSeconds, poolLengthMeters, selectedPrescription, targetMinutes, targetSeconds, timing, user?.id]);
  const cardioSession = useCardioSession({ startConfig, liveMetricValue: metric.value, buildSnapshot, onPersistError: () => setToast("A conexão oscilou. A sessão continua e tentaremos salvar novamente.") });

  useEffect(() => {
    if (!user) return; let mounted = true; setLoading(true);
    loadCardioDashboard(user.id).then((dashboard) => {
      if (!mounted) return; setPrescriptions(dashboard.prescriptions); setRecentSessions(dashboard.recentSessions);
      if (dashboard.openSession) {
        const current = dashboard.openSession; const defaults = activityDefaults[current.activityType]; const recoveredSpeed = current.averageSpeedKmh || speedFromPace(current.averagePaceSeconds) || defaults.speed;
        setActivity(current.activityType); setTiming(current.timing); setTargetMinutes(clamp(Math.round(current.targetDurationSeconds / 60) || 30, 5, 120)); setPaceSeconds(current.averagePaceSeconds || defaults.pace); setSpeedKmh(Number(recoveredSpeed.toFixed(1))); setCadenceRpm(current.cadenceRpm || defaults.cadence); setLaps(current.laps); cardioSession.restore(current); setToast("Sessão recuperada. Toque em retomar para continuar.");
      } else { const first = dashboard.prescriptions[0]; if (first) applyActivity(first.activityType, first); }
      setLoading(false);
    });
  ...
  useEffect(() => { if (cardioSession.syncStatus !== "synced" || !user?.id) return; void loadCardioDashboard(user.id).then((dashboard) => setRecentSessions(dashboard.recentSessions)); }, [cardioSession.syncStatus, user?.id]);
  useEffect(() => { if (cardioSession.phase !== "em_andamento" || cardioSession.elapsedSeconds < targetSeconds || targetReachedRef.current) return; targetReachedRef.current = true; haptic([120, 70, 120]); setToast("Meta atingida! Você pode continuar ou pausar para finalizar."); }, [cardioSession.elapsedSeconds, cardioSession.phase, targetSeconds]);

  const setIntensityValue = (rawValue: number) => { const value = clamp(roundToStep(rawValue, intensityConfig.step), intensityConfig.min, intensityConfig.max); haptic(8); if (activity === "treadmill" || activity === "walk" || activity === "elliptical" || activity === "spinning") { setSpeedKmh(Number(value.toFixed(1))); return; } if (activity === "stairs") { setCadenceRpm(Math.round(value)); return; } setPaceSeconds(Math.round(value)); };
  const selectActivity = (nextActivity: CardioActivity) => { if (cardioSession.phase !== "idle") { setToast("Finalize a sessão atual antes de trocar a modalidade."); return; } const prescription = prescriptions.find((item) => item.activityType === nextActivity) ?? null; applyActivity(nextActivity, prescription); targetReachedRef.current = false; };
  const finish = async () => { haptic(20); await cardioSession.finish(); };
  const resetSession = () => { cardioSession.reset(); setLaps(0); targetReachedRef.current = false; };
  const currentOption = getActivityOption(activity);
  const currentMetrics = metricsForElapsed(cardioSession.elapsedSeconds);
  const distanceLabel = activity === "stairs" ? `${Math.round(currentMetrics.distanceMeters)} degraus` : currentMetrics.distanceMeters >= 1000 ? `${(currentMetrics.distanceMeters / 1000).toFixed(2)} km` : `${Math.round(currentMetrics.distanceMeters)} m`;

  if (authLoading || loading) return <LoadingSplash />;
  if (!user) return <Navigate to="/login" replace />;
  if (landingPath !== "/menu-teste") return <Navigate to={landingPath} replace />;

  if (cardioSession.phase === "finalizado") return <CardioSummary activity={activity} elapsedSeconds={cardioSession.elapsedSeconds} targetSeconds={targetSeconds} distanceMeters={currentMetrics.distanceMeters} calories={currentMetrics.calories} metric={metric} syncStatus={cardioSession.syncStatus === "idle" ? "pending" : cardioSession.syncStatus} validForRanking={cardioSession.validForRanking} reducedMotion={reducedMotion} onRetrySync={cardioSession.retrySync} onAgain={resetSession} onMenu={() => navigate("/menu-teste")} />;
    ...
    <div className="cardio-screen">
      ...
      <main className="cardio-page">
        ...
        <div className="cardio-scroll-area">
          ...
          {activity === "swim" && cardioSession.phase !== "idle" ? <section className="cardio-laps-card"><div><span>VOLTAS</span><strong>{laps}</strong><small>{poolLengthMeters} m cada</small></div><motion.button whileTap={reducedMotion ? undefined : { scale: 0.92 }} type="button" onClick={() => { haptic(12); setLaps((value) => value + 1); }} aria-label="Adicionar uma volta"><CardioPlusIcon /> Marcar volta</motion.button></section> : null}
          <section className="cardio-actions" aria-label="Controles da sessão"><AnimatePresence mode="wait" initial={false}>{cardioSession.phase === "idle" ? <motion.button key="start" layoutId="cardio-primary-action" type="button" className="cardio-primary-action" onClick={() => { haptic(16); void cardioSession.start(); }} disabled={cardioSession.busy} initial={reducedMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? undefined : { opacity: 0, y: -5 }} whileTap={reducedMotion ? undefined : { scale: 0.975 }} aria-label="Iniciar cardio"><CardioPlayIcon /> <span>{cardioSession.busy ? "Preparando..." : "Iniciar cardio"}</span></motion.button> : cardioSession.phase === "em_andamento" ? <motion.button key="pause" layoutId="cardio-primary-action" type="button" className="cardio-primary-action is-running" onClick={() => { haptic(14); void cardioSession.pause(); }} whileTap={reducedMotion ? undefined : { scale: 0.975 }} aria-label="Pausar cardio"><CardioPauseIcon /> <span>Pausar</span></motion.button> : <motion.button key="resume" layoutId="cardio-primary-action" type="button" className="cardio-primary-action" onClick={() => { haptic(14); void cardioSession.resume(); }} whileTap={reducedMotion ? undefined : { scale: 0.975 }} aria-label="Retomar cardio"><CardioPlayIcon /> <span>Retomar cardio</span></motion.button>}</AnimatePresence><AnimatePresence>{cardioSession.phase === "pausado" ? <motion.button type="button" className="cardio-finish-action" initial={reducedMotion ? false : { opacity: 0, height: 0 }} animate={{ opacity: 1, height: 50 }} exit={reducedMotion ? undefined : { opacity: 0, height: 0 }} onClick={() => void finish()} disabled={cardioSession.busy} whileTap={reducedMotion ? undefined : { scale: 0.975 }} aria-label="Finalizar e salvar cardio"><CardioStopIcon /> <span>{cardioSession.busy ? "Salvando..." : "Finalizar e ver resumo"}</span></motion.button> : null}</AnimatePresence></section>
        </div>
      </main>

      {cardioSession.phase === "em_andamento" && cardioSession.elapsedSeconds < targetSeconds ? <TimerOverlay remainingSeconds={Math.max(0, targetSeconds - cardioSession.elapsedSeconds)} totalSeconds={targetSeconds} title={`CARDIO · ${getActivityOption(activity).label}`} subtitle="Sessão em andamento" mediaUrl={getActivityOption(activity).image} mediaAlt={getActivityOption(activity).label} canSkip skipLabel="Pausar cardio" onSkip={() => { haptic(14); void cardioSession.pause(); }} /> : null}

      <AnimatePresence initial={false}>
        {settingsOpen ? <motion.div key="cardio-settings-window" className="cardio-modal-overlay" role="dialog" aria-modal="true" aria-label="Configurações do cardio" variants={accquaOverlayVariants} initial="hidden" animate="visible" exit="exit" transition={accquaOverlayTransition} data-accqua-window-overlay data-accqua-motion-managed>
          <button type="button" className="cardio-modal-backdrop" onClick={() => setSettingsOpen(false)} aria-label="Fechar configurações" />
          <motion.section className="cardio-settings-sheet" variants={accquaWindowVariants.sheet} initial="hidden" animate="visible" exit="exit" transition={accquaWindowTransition} data-accqua-window-surface="sheet">
            ...
            <div className="cardio-setting-group"><strong>Meta de tempo</strong><div className="cardio-stepper cardio-time-stepper"><button type="button" onClick={() => setTargetMinutes((value) => Math.max(5, value - 5))} aria-label="Diminuir meta em cinco minutos"><CardioMinusIcon /></button><span>{targetMinutes} min</span><button type="button" onClick={() => setTargetMinutes((value) => Math.min(120, value + 5))} aria-label="Aumentar meta em cinco minutos"><CardioPlusIcon /></button></div></div>
            <div className="cardio-setting-group"><strong>Quando será feito?</strong><div className="cardio-segmented">{(Object.keys(timingLabels) as CardioTiming[]).map((item) => <button type="button" key={item} className={timing === item ? "active" : ""} onClick={() => setTiming(item)} aria-pressed={timing === item}>{timingLabels[item]}</button>)}</div></div>
            {activity === "spinning" ? <div className="cardio-setting-group"><strong>Cadência da bike</strong><div className="cardio-stepper"><button type="button" onClick={() => setCadenceRpm((value) => Math.max(30, value - 5))} aria-label="Diminuir cadência"><CardioMinusIcon /></button><span>{cadenceRpm} rpm</span><button type="button" onClick={() => setCadenceRpm((value) => Math.min(160, value + 5))} aria-label="Aumentar cadência"><CardioPlusIcon /></button></div></div> : null}
            {activity === "swim" ? <div className="cardio-setting-group"><strong>Tamanho da piscina</strong><div className="cardio-segmented two">{[25, 50].map((length) => <button type="button" key={length} className={poolLengthMeters === length ? "active" : ""} onClick={() => setPoolLengthMeters(length)} aria-pressed={poolLengthMeters === length}>{length} metros</button>)}</div></div> : null}
      ...
      <AnimatePresence initial={false}>
        {historyOpen ? <motion.div key="cardio-history-window" className="cardio-modal-overlay" role="dialog" aria-modal="true" aria-label="Histórico de cardio" variants={accquaOverlayVariants} initial="hidden" animate="visible" exit="exit" transition={accquaOverlayTransition} data-accqua-window-overlay data-accqua-motion-managed>
          <button type="button" className="cardio-modal-backdrop" onClick={() => setHistoryOpen(false)} aria-label="Fechar histórico" />
          <motion.section className="cardio-history-sheet" variants={accquaWindowVariants.sheet} initial="hidden" animate="visible" exit="exit" transition={accquaWindowTransition} data-accqua-window-surface="sheet">
            ...
            <div className="cardio-history-list">{recentSessions.length ? recentSessions.map((item) => { const option = getActivityOption(item.activityType); return <article key={item.id}><img src={option.image} alt="" aria-hidden="true" loading="lazy" /><div><strong>{option.label}</strong><span>{formatClock(item.elapsedSeconds)} • {item.activityType === "stairs" ? `${Math.round(item.distanceMeters)} degraus` : `${(item.distanceMeters / 1000).toFixed(2)} km`} • {item.calories} kcal</span><small>{item.completedAt ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(item.completedAt)) : "Sessão concluída"}{item.validForRanking ? " • Ranking" : ""}</small></div><CardioCheckIcon /></article>; }) : <div className="cardio-history-empty">O primeiro cardio concluído aparecerá aqui.</div>}</div>
          </motion.section>
        </motion.div> : null}
      </AnimatePresence>
