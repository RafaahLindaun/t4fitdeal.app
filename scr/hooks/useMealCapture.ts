...
export function useMealCapture(userId: string, onSaved: () => Promise<void> | void) {
  ...
  const confirm = useCallback(async (override?: { items: MealItem[]; caloriesTotal: number; macros: MealMacros }) => {
    if (!draft || saving) return;
    const finalDraft = override ? { ...draft, ...override } : draft;
    setSaving(true);
    try {
      await saveMeal(userId, {
        imagePath: finalDraft.imagePath,
        items: finalDraft.items,
        caloriesTotal: finalDraft.caloriesTotal,
        macros: finalDraft.macros,
        source: !finalDraft.automaticAvailable || finalDraft.confidence < 0.6 ? "manual" : finalDraft.source,
