...
export async function loadFeedbackPreferences(userId: string): Promise<FeedbackPreferences> {
  const response = await supabase.from("accqua_profile_preferences").select("sound_enabled,vibration_enabled,workout_notifications,class_notifications,activity_reminders,news_notifications,class_reminder_minutes").eq("user_id", userId).maybeSingle();
  const preferences = response.error ? readFeedbackPreferences(userId) : {
    ...
    classReminderMinutes: Math.max(30, Number(response.data?.class_reminder_minutes ?? 120)),
  };
