...
import { classDateTime, loadMyClasses, updateMyGympass } from "./classes";
...
function normalizeProfileDetails(
  ...
  return {
    ...
    gympassNumber: text(raw?.numero_gympass),
...
async function loadProfileRow(userId: string) {
  const supplemental = async () => {
    const response = await supabase
      .from("profiles")
      .select("numero_gympass,matricula_valida_ate")
      .eq("id", userId)
      .maybeSingle();
    return response.error ? {} : ((response.data ?? {}) as Record<string, unknown>);
  };

  const officialRpc = await supabase.rpc("get_my_accqua_profile");
  if (!officialRpc.error && officialRpc.data) {
    const row = Array.isArray(officialRpc.data) ? officialRpc.data[0] : officialRpc.data;
    return row && typeof row === "object"
      ? { ...(row as Record<string, unknown>), ...(await supplemental()) }
      : null;
  }

  const compatibilityRpc = await supabase.rpc("get_my_accqua_profile_v8_6");
  if (!compatibilityRpc.error && compatibilityRpc.data) {
    const row = Array.isArray(compatibilityRpc.data) ? compatibilityRpc.data[0] : compatibilityRpc.data;
    return row && typeof row === "object"
      ? { ...(row as Record<string, unknown>), ...(await supplemental()) }
      : null;
  }
...
async function loadClassRows(_userId: string) {
  const reservations = await loadMyClasses();
  return reservations.map((reservation) => {
    const start = classDateTime(reservation.date, reservation.startTime);
    const end = classDateTime(reservation.date, reservation.endTime);
    ...
    return normalizeActivity({
      ...
      instructor_name: reservation.professorName,
    });
  });
}
...
export async function savePersonalProfile(
  ...
  if (!rpc.error) {
    try {
      await updateMyGympass(input.gympassNumber);
      return {};
    } catch (error) {
      return { error: error instanceof Error ? error.message : "Não foi possível salvar o Gympass." };
    }
  }
