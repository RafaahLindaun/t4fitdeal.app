...
import { loadStudentsMembership, updateStudentGympass } from "./classes";
import { normalizeExerciseMediaKey } from "./exerciseMedia";
...
export type WorkoutStudent = {
  ...
  membershipPaymentDay: number;
  membershipLastPayment: string;
  membershipConfirmedAt: string;
...
export type DeleteStudentAccountResult = {
  deleted: boolean;
  userId: string;
};

export type WorkoutRequiredAlert = {
  id: string;
  studentId: string;
  studentName: string;
  title: string;
...
function humanizeLegacyMediaName(value: string) {
  let decoded = value;
  ...
  const base = (decoded.split("/").pop() ?? decoded)
    ...
    .replace(/\s+/g, " ")
    .trim();
  return base ? base.charAt(0).toUpperCase() + base.slice(1) : "Exercício";
}

function inferLegacyMuscleGroup(key: string) {
  if (/abdominal|crunch|prancha|core|woodchopper/.test(key)) return "Abdômen";
  if (/supino|crucifixo|crossover|peck|flexao-de-braco/.test(key)) return "Peitoral";
  if (/rosca|biceps/.test(key)) return "Bíceps";
  if (/triceps|mergulho|pulley/.test(key)) return "Tríceps";
  if (/puxada|remada|pulldown|pull-up|barra-fixa/.test(key)) return "Costas";
  if (/elevacao-lateral|elevacao-frontal|desenvolvimento|face-pull/.test(key)) return "Ombros";
  if (/agachamento|leg-press|extensora|afundo|passada|hack-squat|step-up/.test(key)) return "Quadríceps";
  if (/flexora|stiff|terra-romeno|nordic|good-morning/.test(key)) return "Posterior de coxa";
  if (/glute|abducao|abdutora|elevacao-pelvica|hip-thrust/.test(key)) return "Glúteos";
  if (/panturrilha/.test(key)) return "Panturrilhas";
  return "Outros";
}

function inferLegacyEquipment(key: string) {
  if (/maquina|machine|extensora|flexora|abdutora|adutora|leg-press|hack/.test(key)) return "Máquina";
  if (/polia|crossover|pulldown|pulley/.test(key)) return "Polia";
  if (/halter|dumbbell/.test(key)) return "Halteres";
  if (/barra|barbell/.test(key)) return "Barra";
  if (/banco|bench/.test(key)) return "Banco";
  if (/elastico/.test(key)) return "Elástico";
  if (/peso-corpo|bodyweight|flexao|prancha|crunch/.test(key)) return "Peso corporal";
  return "";
}

export function mergeExerciseLibraryWithManifest(
  library: ExerciseLibraryItem[],
  manifestFiles: string[],
): ExerciseLibraryItem[] {
  const result = new Map<string, ExerciseLibraryItem>();
  const aliases = new Map<string, ExerciseLibraryItem>();

  // O resultado parte da biblioteca real do banco. A lista fallback serve apenas
  // como dicionário para reconhecer os GIFs legados; ela não cria exercícios
  // fantasma quando o arquivo físico não existe.
  for (const item of library) {
    const identity = normalizeExerciseMediaKey(item.slug || item.name) || item.slug || item.id;
    result.set(identity, item);
  }

  // Fallback primeiro, banco depois: registros reais sempre vencem aliases antigos.
  for (const item of [...FALLBACK_EXERCISE_LIBRARY, ...library]) {
    for (const key of [
      normalizeExerciseMediaKey(item.slug),
      normalizeExerciseMediaKey(item.name),
      normalizeExerciseMediaKey(item.mediaUrl),
    ]) {
      if (key) aliases.set(key, item);
    }
  }

  for (const mediaUrl of manifestFiles) {
    const key = normalizeExerciseMediaKey(mediaUrl);
    if (!key) continue;
    const matched = aliases.get(key) ?? result.get(key);
    if (matched) {
      const currentKey = normalizeExerciseMediaKey(matched.slug || matched.name) || key;
      // Se o exercício já aponta para Storage/URL externa homologada, ela é
      // a mídia canônica. O manifest só corrige/preenche referências locais.
      const preferredMedia = /^https?:\/\//i.test(matched.mediaUrl) ? matched.mediaUrl : mediaUrl;
      result.set(currentKey, { ...matched, mediaUrl: preferredMedia });
      continue;
    }
    const name = humanizeLegacyMediaName(mediaUrl);
    result.set(key, createLibraryExercise(name, inferLegacyMuscleGroup(key), inferLegacyEquipment(key), {
      id: `manifest-${key}`,
      slug: key,
      mediaUrl,
    }));
  ...
  if (!result.size && !manifestFiles.length) {
...
export async function loadSyncedExerciseLibrary(manifestFiles: string[] = []): Promise<ExerciseLibraryItem[]> {
  const current = await loadExerciseLibrary();
  const merged = mergeExerciseLibraryWithManifest(current, manifestFiles);
  if (!isSupabaseConfigured || !manifestFiles.length) return merged;

  const manifestKeys = new Set(manifestFiles.map(normalizeExerciseMediaKey).filter(Boolean));
  const payload = merged
    .filter((item) => manifestKeys.has(normalizeExerciseMediaKey(item.mediaUrl)))
    .map((item) => ({
    slug: item.slug,
    name: item.name,
    ...
    beginner_tip: item.beginnerTip,
    default_sets: item.defaultSets,
    default_reps_min: item.defaultRepsMin,
    default_reps_max: item.defaultRepsMax,
    default_rest_seconds: item.defaultRestSeconds,
  }));

  if (!payload.length) return merged;

  const sync = await supabase.rpc("accqua_sync_legacy_exercise_manifest_v1_4_2", {
    p_items: payload,
  });
  if (sync.error) return merged;
  const canonical = await loadExerciseLibrary();
  return mergeExerciseLibraryWithManifest(canonical, manifestFiles);
}
...
function mapStudent(row: Record<string, unknown>): WorkoutStudent {
  return {
    ...
    gympassNumber: String(row.numero_gympass ?? ""),
    membershipValidUntil: String(row.matricula_valida_ate ?? "").slice(0, 10),
    membershipPaymentDay: Math.max(0, Number(row.matricula_dia_pagamento ?? 0)),
    membershipLastPayment: String(row.matricula_ultimo_pagamento ?? "").slice(0, 10),
    membershipConfirmedAt: String(row.matricula_confirmada_em ?? ""),
    membershipNotes: String(row.matricula_observacao ?? ""),
...
function approvalStatusToStudentStatus(
  value: unknown,
): StudentAccessStatus {
...
async function applyCanonicalAccessStatus(
  students: WorkoutStudent[],
): Promise<WorkoutStudent[]> {
  if (!isSupabaseConfigured || students.length === 0) return students;

  const ids = Array.from(
    new Set(students.map((student) => student.id).filter(Boolean)),
  );
  ...
  if (response.error || !response.data) return students;

  const statusByUser = new Map(
    response.data.map((row) => [
      String(row.user_id),
      approvalStatusToStudentStatus(row.status),
    ]),
  );

  return students.map((student) => ({
    ...student,
    status: statusByUser.get(student.id) ?? student.status,
  }));
}
...
async function applyStudentAvatars(
  ...
  const avatars = await loadResolvedAvatarsByUserIds(
    students.map((student) => student.id),
  );
  return students.map((student) => ({
    ...student,
    avatarUrl: avatars.get(student.id) || student.avatarUrl,
  }));
}
...
async function prepareStudents(
  ...
  const canonical = await applyCanonicalAccessStatus(students);
  const memberships = await loadStudentsMembership(canonical.map((student) => student.id));
  const enriched = canonical.map((student) => {
    const membership = memberships.get(student.id);
    return membership ? {
      ...student,
      gympassNumber: membership.gympassNumber,
      membershipValidUntil: membership.membershipValidUntil,
      membershipPaymentDay: membership.membershipPaymentDay,
      membershipLastPayment: membership.membershipLastPayment,
      membershipConfirmedAt: membership.membershipConfirmedAt,
      membershipNotes: membership.membershipNotes,
    } : student;
  });
  return await applyStudentAvatars(enriched);
}
...
async function prepareStudent(
  ...
  const [resolved] = await prepareStudents([student]);
...
function mapLibraryItem(row: Record<string, unknown>): ExerciseLibraryItem {
  ...
  return {
    ...
    // Build 1.3.6: o banco/Storage é a única fonte de mídia em produção.
    // Não fabricar /gifs/<slug>.gif: no Linux/Vercel isso mascarava 404 por case.
...
export async function createExerciseLibraryItem(
  input: CreateExerciseLibraryInput,
): Promise<ExerciseLibraryItem> {
  if (!isSupabaseConfigured) {
    return createLibraryExercise(
      input.name.trim(),
      input.muscleGroup.trim() || "Outros",
      input.equipment.trim(),
      {
        mediaUrl: input.mediaUrl.trim(),
        beginnerTip: input.beginnerTip.trim(),
        defaultSets: Math.max(1, input.defaultSets),
        defaultRepsMin: Math.max(1, input.defaultRepsMin),
        defaultRepsMax: Math.max(
          Math.max(1, input.defaultRepsMin),
          input.defaultRepsMax,
        ),
        defaultRestSeconds: Math.max(15, input.defaultRestSeconds),
      },
    );
  }

  const rpc = await supabase.rpc("create_accqua_exercise_v6", {
    p_exercise: {
      name: input.name.trim(),
      muscle_group: input.muscleGroup.trim() || "Outros",
      equipment: input.equipment.trim(),
      media_url: input.mediaUrl.trim() || null,
      beginner_tip:
        input.beginnerTip.trim() ||
        "Priorize uma execução segura antes de aumentar a carga.",
      default_sets: Math.max(1, input.defaultSets),
      default_reps_min: Math.max(1, input.defaultRepsMin),
      default_reps_max: Math.max(
        Math.max(1, input.defaultRepsMin),
        input.defaultRepsMax,
      ),
      default_rest_seconds: Math.max(15, input.defaultRestSeconds),
    },
  });
  ...
  if (rpc.error || !rpc.data) {
    throw new Error(
      rpc.error?.message ||
        "Não foi possível salvar o exercício. Execute o SQL accqua_montador_treino_completo_v8.sql no Supabase.",
    );
  }

  const row = Array.isArray(rpc.data) ? rpc.data[0] : rpc.data;
  if (!row || typeof row !== "object") {
    throw new Error("O Supabase não confirmou o novo exercício.");
  }

  return mapLibraryItem(row as Record<string, unknown>);
}
...
export async function searchWorkoutStudents(query: string): Promise<WorkoutStudent[]> {
  ...
  if (!guidedRpc.error && Array.isArray(guidedRpc.data)) {
    return await prepareStudents(
      guidedRpc.data.map((row) =>
  ...
  if (!rpc.error && Array.isArray(rpc.data)) {
    return await prepareStudents(
      rpc.data.map((row) => mapStudent(row as Record<string, unknown>)),
    );
  }
  ...
  const fallbackStudents = (response.data ?? [])
  ...
  return await prepareStudents(fallbackStudents);
}
...
export async function loadExerciseLibrary(): Promise<ExerciseLibraryItem[]> {
  ...
  if (response.error) {
    throw new Error(response.error.message || "Não foi possível carregar a biblioteca de exercícios.");
  }

  return (response.data ?? [])
    .map((row) => mapLibraryItem(row as Record<string, unknown>))
    .sort((a, b) => `${a.muscleGroup}-${a.name}`.localeCompare(`${b.muscleGroup}-${b.name}`, "pt-BR"));
}

export type StaffDeleteResult = {
  action: "deleted" | "deactivated";
  dependencies: number;
};

export async function deleteExerciseLibraryItem(id: string): Promise<StaffDeleteResult> {
  const { data, error } = await supabase.rpc("accqua_staff_delete_exercise_v1_3_6", {
    p_exercise_id: id,
  });
  if (error) throw new Error(error.message);
  const raw = (Array.isArray(data) ? data[0] : data ?? {}) as Record<string, unknown>;
  return {
    action: String(raw.action ?? "deactivated") === "deleted" ? "deleted" : "deactivated",
    dependencies: Math.max(0, Number(raw.dependencies ?? 0)),
  };
}
...
export async function getWorkoutStudentById(
  ...
  return await prepareStudent(
    mapStudent(response.data as Record<string, unknown>),
  );
}
...
export async function updateWorkoutStudentProfile(
  ...
  const payload = {
    ...
    numero_gympass: input.gympassNumber.trim() || null,
  };
  ...
  if (!rpc.error) {
    await updateStudentGympass(studentId, input.gympassNumber);
  ...
  return (await prepareStudent(mapStudent(response.data as Record<string, unknown>)))!;
}
...
export async function setStudentProfessorLink(
  ...
  if (!refreshed || refreshed.linkedProfessorId !== professorId) {
    throw new Error(
      "O vínculo não foi confirmado. Execute o SQL accqua_montador_treino_completo_v8.sql no Supabase.",
    );
  }
...
export async function deleteWorkoutStudentAccount(
  studentId: string,
  confirmation: string,
): Promise<DeleteStudentAccountResult> {
  if (!isSupabaseConfigured) {
    throw new Error("O Supabase não está configurado.");
  }

  const rpc = await supabase.rpc("delete_accqua_student_account_v6", {
    p_student_id: studentId,
  ...
  if (rpc.error) {
    throw new Error(
      rpc.error.message ||
        "Não foi possível apagar a conta definitivamente.",
    );
  }
  ...
  if (result?.deleted !== true) {
    throw new Error("O Supabase não confirmou a exclusão da conta.");
  }

  return {
    deleted: true,
    userId: String(result.user_id ?? studentId),
  };
}
...
export async function setWorkoutStudentAccess(
  ...
  const rpc = await supabase.rpc(
    "set_accqua_app_approval_v4",
    {
      p_user_id: studentId,
      p_status: requestedApprovalStatus,
    },
  );
  ...
  if (rpc.error) {
    ...
    if (
      technicalMessage.includes("set_accqua_app_approval_v4") ||
      technicalMessage.includes("schema cache") ||
      technicalMessage.includes("function")
    ) {
      throw new Error(
        "Execute o SQL accqua_aprovacao_administrativa_v4.sql inteiro no Supabase.",
      );
    }

    throw new Error(
      technicalMessage ||
        "Não foi possível alterar o acesso ao aplicativo.",
    );
  }
  ...
  const persistedStatus = String(response?.status ?? "")
    .trim()
    .toLowerCase();

  if (persistedStatus !== requestedApprovalStatus) {
    throw new Error(
      "O Supabase não confirmou o estado de acesso solicitado.",
    );
  }

  if (response?.persisted !== true) {
    throw new Error(
      "A liberação não foi confirmada no controle de acesso.",
    );
  }

  return {
    status: approvalStatusToStudentStatus(persistedStatus),
    accessMode: "admin_approval",
    alreadyAuthorized: Boolean(
      response?.already_approved ?? false,
    ),
  };
}
...
export async function loadStudentActivityHistory(
  studentId: string,
  ...
  if (!newest.error && Array.isArray(newest.data)) {
    return (newest.data as Record<string, unknown>[]).slice(0, 100).map((row) => ({
      id: String(row.id ?? ""),
      kind: row.activity_kind === "cardio" ? "cardio" : "workout",
      title: String(row.title ?? (row.activity_kind === "cardio" ? "Cardio" : "Treino")),
  ...
  if (!rpc.error && Array.isArray(rpc.data)) {
    return (rpc.data as Record<string, unknown>[]).slice(0, 100).map((row) => ({
      id: String(row.id ?? ""),
      kind: row.activity_kind === "cardio" ? "cardio" : "workout",
      title: String(row.title ?? (row.activity_kind === "cardio" ? "Cardio" : "Treino")),
      performedAt: String(row.performed_at ?? ""),
  ...
  const response = await supabase
    ...
    .select(
      "id,activity_kind,title,performed_at,duration_seconds,calories,completion_percentage,valid_for_ranking",
    )
...
export async function loadWorkoutRequiredAlerts(): Promise<WorkoutRequiredAlert[]> {
  if (!isSupabaseConfigured) return [];

  const response = await supabase.rpc("get_accqua_workout_alerts_v7");

  if (response.error) {
    const message = String(response.error.message ?? "");
    if (
      message.includes("get_accqua_workout_alerts_v7") ||
      message.includes("schema cache") ||
      message.includes("function")
    ) {
      return [];
    }
    throw new Error(message || "Não foi possível carregar os alertas de treino.");
  }
  ...
  return rows.map((row: Record<string, unknown>) => ({
    ...
    studentName: String(row.student_name ?? "Aluno"),
    title: String(row.title ?? "Aluno autorizado sem treino"),
    message: String(
      row.message ?? "Um aluno autorizado ainda precisa de um treino.",
    ),
...
export async function markWorkoutRequiredAlertDelivered(
  notificationId: string,
): Promise<void> {
  if (!isSupabaseConfigured || !notificationId) return;
  await supabase.rpc("mark_accqua_workout_alert_delivered_v7", {
    p_notification_id: notificationId,
  });
}

export async function markWorkoutRequiredAlertRead(
  notificationId: string,
): Promise<void> {
  if (!isSupabaseConfigured || !notificationId) return;
  await supabase.rpc("mark_accqua_workout_alert_read_v7", {
    p_notification_id: notificationId,
  });
}

export async function markStudentWorkoutAlertsRead(
  studentId: string,
): Promise<void> {
  if (!isSupabaseConfigured || !studentId) return;
  await supabase.rpc("mark_accqua_student_workout_alerts_read_v7", {
    p_student_id: studentId,
  });
}

export async function resolveStudentWorkoutAlerts(
  studentId: string,
): Promise<void> {
  if (!isSupabaseConfigured || !studentId) return;
  await supabase.rpc("resolve_accqua_student_workout_alerts_v7", {
    p_student_id: studentId,
  });
}
...
export async function publishAdminProgram(
  ...
  if (rpc.error) {
    throw new Error(
      rpc.error.message ||
        "Não foi possível publicar o treino. Execute o SQL accqua_montador_treino_completo_v8.sql no Supabase.",
    );
  }

  await resolveStudentWorkoutAlerts(input.studentId);
}
  ...
  if (response.error || !response.data) {
    throw new Error(
      response.error?.message ||
        "Não foi possível salvar o modelo. Execute o SQL accqua_montador_treino_completo_v8.sql no Supabase.",
    );
  }
...
export async function loadAdminProgramTemplates(
  ...
  if (response.error) {
    throw new Error(
      response.error.message.includes("workout_program_templates") ||
      response.error.message.includes("relation")
        ? "Execute o SQL accqua_montador_treino_completo_v8.sql no Supabase para criar os treinos rápidos."
        : response.error.message || "Não foi possível carregar os treinos rápidos.",
    );
  }
