...
export default function CalorieRing({ consumed, target, burned, historyAction }: { consumed: number; target: number; burned: number; historyAction?: ReactNode }) {
  ...
  return (
    <section className={`diet-calorie-card ${overflowPercent > 0 ? "is-overflow" : ""}`} aria-label="Balanço calórico do dia">
      <div className="diet-calorie-history-action">{historyAction}</div>
