...
export default function StorageImageUploadGrid({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of selected) {
      ...
      if (!ACCEPTED.has(file.type)) {
        setUploads((current) => [...current, { key, previewUrl, name: file.name, status: "error", message: "Use JPG, PNG ou WebP." }]);
        continue;
      }
      ...
      try {
        ...
        const ext = extFor(compressed.type || file.type);
  ...
  return (
    <section className="storage-image-uploader" aria-label={label}>
      ...
      <input ref={inputRef} className="storage-image-input" type="file" accept="image/jpeg,image/png,image/webp" multiple={multiple} onChange={(event) => void handleFiles(event.target.files)} />
